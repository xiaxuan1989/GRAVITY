...
$\cate{Grp}$ 为所有群构成的范畴，对象之间的态射定义为群同态，态射的合成与恒等态射定义与 $\cate{Set}$ 类似。
...
\[r \in R, a \in I \im r a \in I, a r \in I.\]
...
进而回顾一下，域 $\F$ 上的\textbf{线性空间}其实就是 $\F$-模。由于 $\F$ 是域，选取左模或右模其实无关宏旨。一般取实数域 $\R$ 或复数域 $\C$，相应有实线性空间和复线性空间。显然 $\F$ 自身可以成为 $\F$-线性空间。线性空间的元素称为\textbf{向量}或\textbf{矢量}。其中内部运算称为（逐点）加法，外部运算称为数乘。物理上常称为\textbf{线性叠加原理}，而将矢量画成笔直的箭头。可以看到，$V$ 在赋予某种乘法后是一个代数，可称\textbf{线性代数}，比如内积空间或者张量积空间。当然，线性代数在现在更多指此概念所发展的一套学说。

设线性空间 $V$。某非空 $S\subset V$ 内元素的全体线性组合构成子空间 $\operatorname{span}B$，称为由 $B$ 张成的。若存在线性无关的非空子集 $B$ 使 $\operatorname{span}B=V$，则称 $B$ 是 $V$ 的一个\textbf{基}(basis)或一组\textbf{基矢}(basis vectors)。$|B|<\aleph_0$ 则称 $V$ 是\textbf{有限维的}，可定义维数 $\dim V:=|B|$。易知任意基的元素个数一致，由此维数是良定义的。若 $|B|\geqslant \aleph_0$，也即任意有限矢量组 $\{e_\mu\}_{\mu=1}^n$ 都不构成基，则称 $V$ 是\textbf{无限维的}。典型例子是 $[a, b] \subset \R$ 上连续复函数构成无限维复线性空间 $C[a, b]$，直观上是因为 $[a, b]$ 有无穷多自由度，从而总存在函数不能有限表出。
映射 $A:V\to V$ 称为 $V$ 上的\textbf{算子}、\textbf{算符}(operator)或\textbf{变换}。
有限维里有很多结论不能简单推广至无限维，否定这些结论带来了更多“反直觉”的性质。相对论的物理量主要属于有限维。

对 $\F^X$ 定义如下加法、数乘、零元：$\forall f,g\in \F^X,x\in X,\alpha\in\F$ 有
\[(f+g)(x):=f(x)+g(x),\quad (\alpha f)(x):=\alpha f(x),\quad 0(x):=0,\]
$\F^X$ 就成了线性空间，且维数为 $|X|$。故泛函等价于线性空间上的函数。$\F^X$ 等势于 $\F^{|X|}$。若 $|X|<\aleph_0$，则 $X$ 上函数即有限数列，$X$ 上泛函相当于 $\F^{|X|}$ 上的多元函数。
故我们多考虑无穷自由度的泛函，是无穷维线性空间 $\F^X$ 上的函数。
...
因此基变换和分量变换是互逆的。故相对于 $\{e_\mu\}$ 而言可称 $v\in V$ 为逆变矢量。

\section{对偶空间}

以 $\F$-线性空间为对象、线性映射为态射，构建\textbf{线性空间范畴} $\cate{Vect}(\F)$。
类此定义有限维 $f$ 线性空间范畴 $\cate{Vect}_f(\F)$，它是 $\cate{Vect}(\F)$ 的全子范畴。

置 $f\in\hom(V,W)$。线性变换或线性算子即取 $W=V$ 的 $f\in\operatorname{End} V$。$W=\F$ 时称为线性函数。设 $V$ 的一个基 $\{e_\mu\}$，对 $V$ 中任意矢量 $k^\mu e_\mu$ 有 $f(k^\mu e_\mu)=k^\mu f(e_\mu)$。故其实只要知道 $\{e_\mu\}$ 在 $f$ 下的像组 $\{f(e_\mu)\}$，则 $V$ 中任意矢量在 $f$ 下的像就确定了，即线性映射完全由基的作用效果确定。显然 $A\in\hom(U,V),B\in\hom(V,W)$ 的复合也线性，即 $BA\in\hom(U,W)$。

假若 $f$ 还是双射，则 $f\in\mathrm{Isom}(V, W)$ 且 $f^{-1}\in\hom(W,V)$。换言之，$V,W$ 的线性双射为同构。有限维时易证 $U\cong V$ 等价于 $\dim U=\dim V$。易知 $f$ 是单射时，核为 $\ker f:=\{\alpha\in V:f(\alpha)=0\}=\{0\}$；$f$ 是满射时像为 $\Im f=W$。秩为 $\rank f:=\dim \Im f$，可证满足 $\dim \ker f + \dim \Im f = \dim V$。故若 $f$ 单射，则 $\Im f\cong V$。对线性变换而言，这说明单射等价于满射。换言之，满秩线性变换乃 $V$ 自同构。

对任意线性空间 $V,W$，只要对 $\hom(V,W)$ 定义如下加法、数乘、零元：$\forall f,g\in \hom(V,W),v\in V,\alpha\in\F$ 有
\[(f+g)(v):=f(v)+g(v),\quad (\alpha f)(v):=\alpha f(v),\quad 0(v):=0,\]
则 $\hom(V,W)$ 成为了线性空间。
考虑对偶函子 $(\oo)^*:\cate{Vect}(\F)^\mathrm{op}\to \cate{Vect}(\F)$。
可以验证它是忠实的。
$V^*=\hom(V,\F)$ 是线性空间，故称为 $V$ 的\textbf{对偶空间}。$\omega\in V^*$ 称为\textbf{对偶矢量}，由于 $\hom(V,\F)\subset\F^V$，也可称\textbf{线性泛函}。
比如，行线性空间就是列线性空间的对偶空间。线性映射 $f\in\hom(V,W)$ 诱导出拉回 $f^*\in\hom(W^*, V^*)$ 使 $f^*\omega=\omega f$。

下证有限维时 $V\cong V^*$。
考虑从原来的 $V$ 来产生 $V^*$ 的基。假设 $\{e_\mu\}$ 是 $V$ 的一个基，则定义 $V^*$ 中有一组元素 $\{e^\mu\}$ 满足
\eq{ e^\mu(e_\nu):=\delta^\mu_\nu,}
...
$V,V^*$ 间的同构当然不唯一。注意到 $\{e_\mu\}$ 到其对偶基 $\{e^\mu\}$ 的线性映射就是同构，那这会随着 $\{e_\mu\}$ 的不同选取而产生不同的同构。
若要有充分理由挑选出特殊同构，需另加结构限制之。
比如我们学过的度规就建立了 $V,V^*$ 的特殊同构。

然而对偶操作不必继续“套娃”。用基底展开后有 $\omega(A)=\omega_\mu A^\mu=A^\mu\omega_\mu$，这说明对偶矢量与矢量之间有天生的(god-given)乘法。映射像乘法一样可交换，矢量也可成为对偶矢量的线性函数，于是 $V,V^**$ 在某种意义上是一样的。
具体地说，考虑双对偶函子 $(\oo)^{**}:\cate{Vect}(\F)\to\cate{Vect}(\F)$，则 $V^{**}=\hom(V^*,\F)$。
为满足“天生乘法”，只需定义 $\eta_V:V\to V^{**}$ 使 $A\mapsto [\omega\mapsto\omega(A)]$，也即$\eta_V A(\omega):=\omega(A)$。$\eta_V A$ 也可记 $A^{**}$。
对任意线性 $f:V\to W$，不难检查以下图表
\[\begin{tikzcd}
    V \arrow[r, "\eta_V"] \ar[d, "f"'] & V^{**} \arrow[d, "f^{**}"] \\
    W \arrow[r, "\eta_W"] & W^{**}
\end{tikzcd}\]
是交换的。可见 $\eta=\{\eta_V\}$ 就是自然变换 $\id\Rightarrow (\oo)^{**}$。
容易看出 $\eta_V:V\to V^{**}$ 总是单射，事实上可证 $\eta$ 是双射当且仅当 $V$ 有限维。
一切限制到全子范畴 $\cate{Vect}_f(\F)$，遂有同构 $\eta_V\in\mathrm{Isom}(V,V^{**})$，即自然同构 $V=V^{* *}$。
或写作
\[\eta: \id_{\cate{Vect}_f(\F)} \xRightarrow{\sim} DD^\mathrm{op}.\]
同一式子在相反范畴中诠释，便是 $\id_{\cate{Vect}_f(\F)^\mathrm{op}} \xRightarrow{\sim} D^\mathrm{op}D$。
故 $D: \cate{Vect}_f(\F)^\text{op} \to \cate{Vect}_f(\F)$ 是范畴等价，$D^\text{op}: \cate{Vect}_f(\F) \to \cate{Vect}_f(\F)^\text{op}$ 则是其拟逆。
注意到 $f$ 取决于对基的作用效果，可见“天生乘法”导致了一种同构，它不依赖于基的人为选取，这便是“自然”的含义。
认为 $A=A^{**}$ 后可写成 $\omega(A)=A(\omega)$，数学上常写成 $\langle\omega,A\rangle$。
作为对比，可证不存在 $i_V\in\mathrm{Isom}(V,V^*)$ 使下图
\[\begin{tikzcd}
...
\begin{figure}[ht]
    ...
    \label{fig:vending}
\end{figure}
如图 \ref{fig:vending}，设想度规 $g$ 是一台具有两个槽(slat)的售货机，往槽中投入足够数量的一元纸币可获得一件商品，比如一听可乐。设某件商品需要两元纸币，则投入过程中，显示屏会不断改变数值以提示用户待支付的剩余金额，直至最终获得商品。这个过程类似于：
\[
    g(\oo,\oo)\im g(A,\oo)\in V^*\im g(A,B)\in\R.  
\]
...
比如，$V$ 上的线性变换就是输入矢量且输出矢量，即 $X^\mu=T^\mu{}_\nu Y^\nu$，而矢量是对偶矢量的函数，因此线性变换又可看成矢量和对偶矢量的双线性函数，即 $(1,1)$-张量 $T^\mu{}_\nu$。比如 $\id_V$ 给出 $\delta^i_j A^j=A^i$，而 $\delta^i_j$ 单独看有上下槽。物理上有一例：应力张量涉及某个面上的压力，因此必定要先给定一个面法矢，这样就产生对应的一个作用力。

综上，分量就是 $g_{ij}:=g(e_i,e_j)$，$(g_{ij})$ 是 $g$ 的一个表示矩阵，也就有 $g(A, B)=g_{\mu\nu} A^\mu B^\nu$。
张量可展开成线性组合，这涉及从矢量的基构造张量的基。将运算符记作 $\otimes$，并期望表示 $g=g_{ij} e^i\otimes e^j$。尝试作用于 $A^i,B^j$，则 $g_{ij}A^i B^j=g_{ij} e^i(A)e^j(B)=g_{ij}e^i\otimes e^j(A,B)$，故直接希望 $e^i\otimes e^j(A,B):= e^i(A) e^j(B)$，即 $\otimes$ 的结果与相应矢量的作用相当于各自分开作用再相乘。
换言之，$A\otimes B(\omega_1,\omega_2):=A(\omega_1)B(\omega_2)$，这样 $A\otimes B$ 的分量是只是简单拼凑 $A^i B^j$。
此即我们所熟知的张量代数的映射语言。

\begin{definition}
    设 $\F$-线性空间 $\{X_i\}_{i=1}^k,W$。若
    $T:X_1\times\cdots\times X_k\to W$
    对任何 $X_i$ 都线性，则称\textbf{多重线性映射}。常取如下情形：
    \eq{
        T:\underbrace{V^*\times\cdots\times V^*}_{k\in\N}\times\underbrace{V\times\cdots\times V}_{l\in\N}\to \F,
    }
    称为 $V$ 上的一个\textbf{多重线性函数}或\textbf{张量}。如上即 $(k,l)$-张量，构成张量空间
\eq{
    \hom(\underbrace{V^*\times\cdots\times V^*}_{k\in\N}\times\underbrace{V\times\cdots\times V}_{l\in\N},\F)=:V^k_l.
}
...
\begin{remark}
    用分号区分上下槽，如 $T(\oo;\oo,\oo)\in V^1_2$ 可输入 $\omega,v_1,v_2$ 得 $T(\omega;v_1,v_2)\in\F$。$T$ 视为全部槽为空时的简写。
\end{remark}
...
\eq{\mathrm{C}_{j}^{i} T:=T \big(\cdots,\hl{red}{e^{h}},\cdots ; \cdots,\hl[b]{blue}{e_{h}},\cdots\big),}
\hn{-2}{-1}{red}{第 $i$ 上槽}
...
缩并其实是求迹的映射版本，因此数学家会混记为 $\tr$。如线性变换 $T$ 在任意基底下矩阵的迹 $\tr T$ 就是 $(1,1)$-张量缩并，是基不变的标量。但两种符号的逻辑稍许不同：$\tr$ 是写出缩并后张量类型，$\c$ 是指明所缩指标位置。假设 $T$ 是 $(1,3)$-张量，定义 $S:=\mathrm{C}_2^1T$，分量写法是 $S_{ij}=T^{k}{}_{ikj}$，但数学上记作 $S=\tr^0_2 T$。并且 $\tr$ 的用法有时并不唯一，比如 $S_{ij}$ 求迹必须借助升指标得 $g^{ij}S_{ij}$，数学上写作 $\tr_{g}S$。
将分量语言改写为映射语言时，只需注意“先张量积再缩并”可产生各种类型的新张量。可从映射语言直接看出，其结果相当于张量对矢量、对偶矢量的作用。比如，置 $T \in V^k_l,v \in V$，则
\begin{align*}
    \mathrm{C}_{\cdots}^{k+1}(T \otimes v)&=T \otimes v(\cdots,e^{\mu} ; \cdots,e_{\mu},\cdots)=T(\cdots;\cdots,e_{\mu},\cdots)v(e^\mu)\\
    &=T(\cdots;\cdots,v(e^\mu)e_{\mu},\cdots)=T(\cdots;\cdots,v,\cdots);
\end{align*}
同理 $T$ 对 $\omega\in V^*$ 作用其实即 $\mathrm{C}_{l+1}^{\cdots}(T \otimes \omega)$。
换言之，作用就是先积后并。
...
\eq{
\operatorname{S} T:=\frac{1}{k!}\sum_{\sigma\in S_k} \sigma T, \quad\A T:=\frac{1}{k!}\sum_{\sigma\in S_k} \sgn\sigma\,\sigma T,
}
...
\begin{theorem}
    易证 $T=\operatorname{S} T\iff T=\sigma T$；$\omega=\A\omega\iff\omega=\sgn\sigma\,\sigma\omega$。
\end{theorem}

\begin{theorem}
    置 $f\in V^0_k,g\in V^0_l$，则 $\A(\A f\otimes g)=\A(f\otimes\A g)=\A(f\otimes g)$。证明思想与第 \ref{sec:tensor} 节的方法一致。
\end{theorem}
...
专注于 $W=\F$，则 $f$ 为双线性函数、$f_\flat(\alpha)$ 为对偶矢量、$f_\flat\in\hom(V,V^*)$。
...
相对论主要用实线性空间。度规是 $\F=\R$ 的特例：

\begin{definition}
    $V$ 上一个对称、非退化的双线性实函数 g 称为 $V$ 上的一个\textbf{度规}。 $g_\flat:V\to V^*$ 称作\textbf{音乐同构}(musical isomorphism)。度规逆 $g^{-1}:=g^{ij} e_i\otimes e_j$，或用 $\mathrm{C}_1^2(g^{-1}\otimes g)=\id_V$ 定义。
\end{definition}

\begin{remark}
    部分书也把音乐同构称为“典范同构”。但为资区分，分别使用“自由”“音乐”两词而避免“典范”。
\end{remark}

\begin{definition}
    \textbf{升降指标}操作本质为音乐同构，故是相对于某度规而言的。比如 $(1,1)$-张量 $T$ 的降为
    $\mathrm{C}_1^1(g\otimes T)=T_\flat$。升指标用度规逆，即 $\mathrm{C}_1^1(g^{-1}\otimes T)=T^\sharp$。就矢量、对偶矢量而言，可将音乐同构称为\textbf{度规对偶}：$A\in V$ 的降为 $A_\flat:=g_\flat A$；$\omega\in V^*$ 的升是 $\omega^\sharp:=g^{-1}(\omega,\oo)$。
\end{definition}
...
\begin{definition}
    ...
    \end{theorem}
    \begin{theorem}
    对非退化条件中取 $g=f$ 可有：$\forall g\in V,(f,g)=0\im f=0$。
    \end{theorem}
    \begin{example}
...
\begin{definition}
    置内积空间 $V$。对 $f,g\in V$ 的定义通常距离 $d(f, g):=\sqrt{(f-g, f-g)}$，便得到\textbf{度量线性空间}。该度量诱导拓扑之后（见定义 \ref{def:metric_topo}），又成了\textbf{拓扑线性空间}。
\end{definition}
...
\end{definition}
\begin{remark}
    有限维内积空间 $V$ 上的线性函数必连续，故 $\hom(V,\F)$ 等价于此处定义的对偶空间。
\end{remark}
...
\[\operatorname{sym} T:=k!\operatorname{S} T=\sum_{\sigma\in S_k} \sigma T,\quad \alt T:=k!\A T=\sum_{\sigma\in S_k} \sgn\sigma\,\sigma T,\]
...
\begin{definition}
    矢量和形式的缩并 $i_v \omega:=\mathrm{C}_1^1 (v\otimes\omega)=\omega(v,\cdots)$ 称为\textbf{内导数}、\textbf{内缩}或\textbf{内乘}(interior product)，分量表述为
    \eq{
...
举一例很重要的张量密度。某 $\{e_\mu\}$ 的 Levi-Civita 符号为
\eq{
    \varepsilon_{i_1\cdots i_n}:=\det(\delta_{i_n}^{j}).
}
其中 $\dim V=:n$。显然 $\varepsilon_{i_1\cdots i_n}=\varepsilon_{[i_1\cdots i_n]}$。二维方阵 $(M^{ij})$ 的行列式总可表成
\eq{\label{eq:det-epsilon}
    \det(M^{ij})= \varepsilon_{i_1\cdots i_n}M^{i_1 1}\cdots M^{i_n n}.
}
...
\textit{用赝张量构造张量的办法是利用度规}，而此张量只在正交归一基中表为此赝张量，如协变导数的度规适配性。约定某正交归一基 $\{\zeta_\mu\}$ 是右手的，\textbf{Levi-Civita 张量}为 $\epsilon=\epsilon_{i_1\cdots i_n} e^{i_1}\otimes\cdots\otimes e^{i_n}=\varepsilon_{j_1\cdots j_n} \zeta^{j_1}\otimes\cdots\otimes \zeta^{j_n}$，记 $e_\mu=\Pi^\nu{ }_\mu \zeta_\nu$，则
\eq{
...
\eq{
    \sgn g=(-1)^s,\quad |g|=\tilde J^{-2}\im \epsilon_{1\cdots n}=1/\tilde J=\sgn\tilde J\sqrt{|g|}.
}
由于 $\dim V=n$，故
\begin{align}
...
经常还要涉及到其指标缩并：
\eq{
    \delta^{j_{k}j_{k+1}\cdots j_n}_{j_{k}i_{k+1}\cdots i_n}= k \delta^{j_{k+1}\cdots j_n}_{i_{k+1}\cdots i_n}\im \delta^{j_1\cdots j_k j_{k+1}\cdots j_n}_{j_1\cdots j_k i_{k+1}\cdots i_n}=  k !  \delta^{j_{k+1}\cdots j_n}_{i_{k+1}\cdots i_n}.
}
...
\]
可记 $\omega=A\wedge B$，$\star\omega=A\times B$，这里忽略度规升降。$\star$ 称\textbf{星算子}(star)，则 $A\times B=\star(A\wedge B)$，分量为 $(\star\omega)_k=\epsilon_{ijk} A^{i}B^{j}=\frac{1}{2}\omega^{ij}\epsilon_{ijk}$。
