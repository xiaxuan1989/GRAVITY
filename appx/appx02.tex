...
\begin{itemize}
    ...
    \item $U,V\in\mathscr{T}\im U\cap V\in\mathscr{T}$。或等价地对有限集 $\{A_{i}\} \subset \mathscr{T}$，$\bigcap_{i} A_{i} \in \mathscr{T}$。
\end{itemize}
...
\begin{proof}
    $A=A^\circ\in\mathscr T$；反之 $A\in\mathscr T,A\subset A\im A\subset A^\circ\iff A=A^\circ$。

    $A=A^\circ\iff\forall x\in A,\exists O\in\mathscr T$ 使 $x\in O\subset A\iff\forall x\in A, A\in\mathscr N(x)$。
    
    $\forall x\in A$ 下，$\exists U\in\mathscr N(x)$ 使 $U\subset A\im \exists O\in\mathscr T$ 使 $ x\in O\subset U\subset A \im A\in\mathscr N(x)$；反之 $\exists A\in\mathscr N(x)$ 使 $A\subset A$。
\end{proof}
...
\end{theorem}
\begin{proof}
    给定 $x\in\bar A$，假设 $\exists U \in\mathscr N(x),U \cap A=\mt$。则 $\exists O\in\mathscr T$ 使 $x\in O\subset U\im A\subset X \backslash U\subset X \backslash O$，$X \backslash O$ 为闭集 $\im \bar{A} \subset X \backslash O\iff O \cap \bar{A}=\mt$，矛盾；
    给定 $\forall U\in\mathscr N(x),U\cap A\ne\mt$，假设 $x\in X \backslash \bar{A}$。则 $X \backslash \bar{A}\in\mathscr T\im \exists X \backslash \bar{A}\in\mathscr N(x),(X \backslash \bar{A}) \cap A=A \backslash \bar{A}=\mt$，矛盾。
\end{proof}
...
\begin{definition}
    ...
    若 $\exists x \in X$，$\forall U\in\mathscr N(x),\exists N\in\N$，使 $n>N\im x_n\in U$，则称 $\{x_n\}$ 有\textbf{极限}或\textbf{收敛}于 $x$。
\end{definition}
