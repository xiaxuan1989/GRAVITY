...
$2^X$ 的子集称为\textbf{子集族}(collection of subsets)，全体子集族构成 $2^{2^X}$。欲对 $X$ 中每个点挑选⼀族⼦集作为其邻域(neighbourhood)，即邻域族，就是给定映射 $\topo N:X\to 2^{2^X}$ 且 $\topo N(x)\ne \mt$。
...
\begin{definition}
    \textbf{开映射} $f:X\to Y$ 把开集送到开集：$A\in\topo T_X\im f[A]\in\topo T_Y$。闭映射定义同理。
\end{definition}
...
\begin{proof}
取任意开集 $U$，同胚 $f:U\to f[U]$。因 $f^{-1}$ 连续，开集 $f^{-1}[f[U]] = U$ 的原像 $f[U]$ 也开。同理可证同胚也是闭映射。
\end{proof}

\begin{definition}
置 $(X,\topo T)$ 和 $A\subset X$。
\textbf{内部}为开集 $A^\circ:=\bigcup_{O\in\topo T\cap 2^A} O$，也即含于 $A$ 的最大开集。
\textbf{闭包}(closure)为闭集 $\bar{A}:=\bigcap_{X\backslash B\in\topo T,A\subset B} B$，也即包含 $A$ 的最小闭集。
...
\end{theorem}
\begin{proof}
    根据定义，$A^\circ:=\bigcup_{X\backslash F\in\topo T,X\backslash F\subset A} X\backslash F=X\backslash\bigcap_{X\backslash F\in\topo T,X\backslash A\subset F} F=X\backslash\overline{X\backslash A}$。
    替换即可。
\end{proof}

\begin{theorem}
    $A$ 为开集 $\eqto A=A^\circ\eqto\forall x\in A,A\in\topo N(x)\eqto\forall x\in A,\exists U\in\topo N(x)$ 使 $U\subset A$。
\end{theorem}

\begin{proof}
    $A=A^\circ\in\topo T$；反之 $A\in\topo T,A\subset A\im A\subset A^\circ\iff A=A^\circ$。

    $A=A^\circ\iff\forall x\in A,\exists O\in\topo T$ 使 $x\in O\subset A\iff\forall x\in A, A\in\topo N(x)$。
    
    $\forall x\in A$ 下，$\exists U\in\topo N(x)$ 使 $U\subset A\im \exists O\in\topo T$ 使 $ x\in O\subset U\subset A \im A\in\topo N(x)$；反之 $\exists A\in\topo N(x)$ 使 $A\subset A$。
\end{proof}
...
\begin{theorem}
    $x\in\bar A\iff\forall U\in\topo N(x),U\cap A\ne\mt$。
\end{theorem}
\begin{proof}
    给定 $x\in\bar A$，假设 $\exists U \in\topo N(x),U \cap A=\mt$。则 $\exists O\in\topo T$ 使 $x\in O\subset U\im A\subset X \backslash U\subset X \backslash O$，$X \backslash O$ 为闭集 $\im \bar{A} \subset X \backslash O\iff O \cap \bar{A}=\mt$，矛盾；
    给定 $\forall U\in\topo N(x),U\cap A\ne\mt$，假设 $x\in X \backslash \bar{A}$。则 $X \backslash \bar{A}\in\topo T\im \exists X \backslash \bar{A}\in\topo N(x),(X \backslash \bar{A}) \cap A=A \backslash \bar{A}=\mt$，矛盾。
\end{proof}
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item \textbf{支撑集}(support)：$\supp f:=\overline{\operatorname{coz}f}
  = X\backslash (Z(f))^\circ$。
    \end{itemize}
    更方便地，要求 $Y$ 是拓扑空间且 $\{0\}$ 是闭集（$Y=\R,\C$ 当然满足），从而可谈及连续 $f$。这样 $Z(f)=f^{-1}\{0\}$ 闭，$\operatorname{coz} f$ 开。
\end{definition}
...
这与 $\R$ 的通常拓扑类似，都是通过囊括空集和“任意并”操作来生成拓扑。这一方法可总结如下。取 $\topo{B} \subset 2^X$，首先它能覆盖整个 $X$：$\bigcup_{U\in\topo{B}}U=X$，其次保持“有限交”性质：对任意有限集 $\{U_i\}\subset\topo{B}$，$\exists\topo{F}\subset \topo{B}$ 使 $\bigcup_{U\in\topo{F}}U=\bigcap_{i} U_i$，就称$\topo{B}$ 为\textbf{拓扑基}。
$\topo{T}=\left\{\bigcup_{U\in\topo{F}}U:\topo{F}\subset\topo{B}\right\}$ 称为 $\topo{B}$ 生成的拓扑。\textbf{拓扑子基}的元素所有可能的有限交构成拓扑基。

许多空间往往都难以直接想象或处理，我们可考虑将它分解为简单直观的部分，再重组回去，就能描述复杂的拓扑空间。
设 $\tilde X=\{[x]:x\in X\}$ 是 $(X,\topo T)$ 在某个等价关系下的分割或商集(quotient set)。对其规定如下拓扑：映射 $\pi: X \to \tilde X,x\mapsto [x]$ 称为\textbf{自然映射}或\textbf{典范投影}，取 $\tilde{\topo T}=\{U \subset Y:\pi^{-1}(U)\in\topo T\}$，称为\textbf{商拓扑}，$(\tilde X,\tilde{\topo T})$ 称为\textbf{商空间}。可见，$\pi$ 就好比用来粘合拓扑空间的胶水。
...
\begin{definition}
    ...
    总可用局部性质构造拓扑：全体开球 $\{B(x,\epsilon):x\in X,\epsilon >0\}$ 就是一个拓扑基，其生成拓扑 $\topo T_d$ 就是 $(X,d)$ 的\textbf{度量诱导拓扑}，构成 $(X,d,\topo T_d)$。称 $(X,\topo T_d)$ \textbf{可度量}。
\end{definition}
...
\begin{definition}
    置 $(X,\topo T)$。
    $\topo C\subset 2^X$ 称为 $A\subset X$ 的\textbf{覆盖}(cover)，若 $A\subset\bigcup_{O\in\topo C}O$。$\topo C\subset\topo T$ 时称\textbf{开覆盖}。
...
\begin{definition}
    \textbf{第二可数}或 $C_2$ 的拓扑空间上存在可数拓扑基。\textbf{Lindel\"of} 的拓扑空间的任意开覆盖都存在可数子覆盖。
\end{definition}
...
\begin{definition}
    拓扑空间 $X$ 称为 $\mathbf{T_2}$ 或 \textbf{Hausdorff 的}，若对任意不同的两点 $x,y \in X$，存在 $x$ 的开邻域 $A$ 与 $y$ 的开邻域 $B$，使得 $A \cap B=\mt$。
\end{definition}
...
\begin{eg}
    一个更为地道的非 $T_2$ 空间：考虑正整数组成的空间 $\N_+$，开集取成 $\mt,\N_+$ 以及集合 $\{1,2,3,\cdots,n\}$。这个空间既非 $T_2$ 亦非紧致的。
\end{eg}

\begin{definition}
拓扑空间 $X$ 称为 $T_3$，若满足
\begin{itemize}
  \item $X$ 是 $T_1$ 空间（单点集闭）；
  \item \textbf{正则}(regular)：对任意点 $x\in X$ 与任意不含 $x$ 的闭集 $F\subset X$，
        存在互不相交的开集 $U,V$ 使得
        \[
          x\in U,\qquad F\subset V,\qquad U\cap V=\varnothing .
        \]
\end{itemize}
\end{definition}
$X$ 正则 $\iff$ 对任意 $x\in X$ 与任意包含 $x$ 的开集 $O$，存在开集 $U$ 使得
$x\in U, \bar U \subset O$
点与不交闭集可被不交开集分开。
这就是你在单位分解/bump 函数构造里常看到的“取 $V$ 使 $\overline V\subset U$”那一步。

\begin{definition}[正规空间]\label{def:normal}
拓扑空间 $X$ 称为\textbf{正规}(normal) $T_4$，若满足
\begin{itemize}
  \item $X$ 是 $T_1$ 空间（单点集闭）；
  \item 对任意两个不交闭集 $F,G\subset X$，存在互不相交的开集 $U,V$ 使得
  \[
    F\subset U,\qquad G\subset V,\qquad U\cap V=\varnothing .
  \]
\end{itemize}
\end{definition}
$X$ 正规 $\iff$ 对任意闭集 $F\subset X$ 与任意开集 $O\supset F$，存在开集 $U$ 使得 $F\subset U, \bar U \subset O$
（这个等价式常被叫作“正规空间的收缩(open shrinking)性质”。）
...
可以保证，局部定义的无穷和/无穷拼接，在每点附近变成有限和。

局部有限就是让“无穷和 = 局部有限和”，从而一切光滑操作合法。
...
\begin{definition}
覆盖 $\topo V$ 称\textbf{局部有限}(locally finite)，若 $\forall x\in X$，$\exists x$ 的邻域 $O\in\topo N(x)$
使得 $O$ 只与 $\topo V$ 中有限多个集合相交：$|\{V\in\topo V:\ O\cap V\neq\varnothing\}|<\aleph_0$。
\end{definition}
...
%%%
\begin{remark}
    更原始的定义是称“任意开覆盖都存在开星加细”：称 $\topo V$ 是覆盖 $\topo U$ 的星加细，对每个 $V\in\topo V$，存在 $U\in\topo U$ 使 $\operatorname{St}(V,\topo V):=\bigcup\{O\in\topo V:\ O\cap V\neq\varnothing\}\subset U$。
    但“局部有限的开加细”最实用。
\end{remark}

``每个开覆盖都有副紧细化''拓扑空间 $X$ 称为\textup{副紧}(paracompact)，若对任意开覆盖 $\topo U$，
都存在一个\textup{开加细} $\topo V$，使得 $\topo V$ 作为一个拓扑空间（带子空间拓扑）
是\textup{紧致化可分离/局部有限可加和}——更常用且等价的表述是：$\topo V$ 有一个\textup{局部有限的开加细}。


（在流形上，我们常直接引用：流形是$C_2$ $T_2$，从而度量化并且仿紧，
故任意开覆盖都有局部有限开加细。）
...
\end{theorem}
\begin{proof}
    下面对一般度量空间 $(X,d)$ 证明副紧性：任取开覆盖 $\mathcal U$，要构造一个局部有限开细化。

对每个 $x\in X$，选取 $U_x\in\mathcal U$ 使 $x\in U_x$。
因为 $U_x$ 开，存在 $r(x)>0$ 使得开球 $B(x,2r(x))\subset U_x$。
令
\[
\mathcal B:=\{B(x,r(x)):\ x\in X\},
\]
则 $\mathcal B$ 是 $\mathcal U$ 的开细化（因为 $B(x,r(x))\subset B(x,2r(x))\subset U_x$），但未必局部有限。
为此我们把这些球按半径分层：
...
\qquad
\mathcal B_n:=\{B(x,r(x)):\ x\in X_n\}.
\]
则 $\mathcal B=\bigcup_{n=1}^\infty \mathcal B_n$，且每层的半径都有统一下界 $>2^{-(n+1)}$。
...
于是对每个 $n$，族
\[
\mathcal V_n:=\{B(s,2^{-n}) : s\in S_n\}
\]
覆盖 $X_n$（因为若 $x\in X_n$，取 $s\in S_n$ 满足 $d(x,s)<2^{-(n+2)}<2^{-n}$）。

\emph{关键：$\mathcal V_n$ 是局部有限的。}
...
最后令
\[
\mathcal V:=\bigcup_{n=1}^\infty \mathcal V_n.
\]
则 $\mathcal V$ 覆盖 $X$（因为 $X=\bigcup_n X_n$，且每个 $X_n$ 被 $\mathcal V_n$ 覆盖），并且是可数并联的局部有限族：
对任意点 $p$，在每个固定层 $n$ 上只有有限多个集合与某小邻域相交，
再取更小邻域即可保证跨所有层也只与有限多个相交，从而 $\mathcal V$ 局部有限。

还需验证它细化原覆盖 $\mathcal U$：对 $B(s,2^{-n})\in\mathcal V_n$，
由于 $s\in X_n$，有 $r(s)>2^{-(n+1)}$，从而 $2^{-n}\le 2r(s)$，于是
\[
B(s,2^{-n})\subset B(s,2r(s))\subset U_s\in\mathcal U.
\]
故 $\mathcal V$ 是 $\mathcal U$ 的局部有限开细化。
因此任意度量空间副紧。

将 $X$ 取为第二步得到可度量化的 $M$，即得 $M$ 副紧，从而仿紧。证毕。
\end{proof}

很多漂亮的等价刻画需要 $T_2$（常常还要正规）
例如你前面截图那种“星细化”“存在分割（partition of unity）”“正规性”等等，常见定理会写成
“paracompact $T_2$ $\Rightarrow$ normal”等。没有 $T_2$，这些结论可能不成立或需要改写。
在微分几何里通常默认流形是 $T_2$（且$C_2$）
因此大家谈副紧时，往往默认讨论的是 “paracompact $T_2$ space”，因为这是保证后续工具（bump 函数、单位分解、向量丛的好性质等）顺畅工作的自然语境。

\begin{theorem}
    仿紧 $\wedge T_2\im T_4$ 
\end{theorem}
而正规性正是 Urysohn 引理、Tietze 延拓、以及各种“用函数分离集合”的工具的前提。

二可数流形 $\Rightarrow$ 度量化 $\Rightarrow$ 副紧

副紧 + $T_2$ + 流形 bump 引理 $\Rightarrow$ 单位分解

你要的“任意开覆盖存在单位分解”这件事，严格说依赖的不是纯拓扑副紧，而是“流形 + $T_2$ + 副紧（在流形里通常由$C_2$/度量化推出）”，这样才能保证有足够的 bump 函数和良好细化来做归一化。
...
\begin{definition}
    ...
    给定一拓扑空间 $X$，定义一等价关系如下：$x \sim y$ 当且仅当 $x,y$ 属于 $X$ 的同一个连通子空间。则每个等价类称为 $X$ 的一个\textbf{连通分支}
\end{definition}

\begin{theorem}
    连续映射保持连通：$f:X\to Y$ 连续时，$A\subset X$ 连通 $\im$ $f[A]$ 连通。
\end{theorem}
%% 有问题
\begin{proof}
假使 $f[A]$ 不连通，则存在不交非空开集 $U,V\subset Y$ 使
\[
f(A)\subset U\cup V,\quad f(A)\cap U\neq\varnothing,\quad f(A)\cap V\neq\varnothing.
\]
于是
\[
...
\begin{definition}
    拓扑空间的\textbf{局部地}具有某个性质，意指任意点都存在邻域（或开邻域、包含该点的开集）具有这种性质。比如，\textbf{局部路径连通}就是任意点都存在邻域路径连通，即 $\forall x\in X$ 都存在开集 $O\ni x$ 路径连通。类似地有\textbf{局部可度量}。\textbf{局部紧}是任意点都存在\textbf{邻域}相对紧（闭包紧）。
\end{definition}
...
%% 没区分子拓扑
\begin{proof}
置 $f:X\to Y$ 连续开。取 $A\subset X$ 局部路径连通。 
...
\begin{theorem}
    连通 $\wedge$ 局部路径连通 $\im$ 路径连通
\end{theorem}
\begin{proof}
    置 $(M,\topo T)$ 连通 $\wedge$ 局部路径连通。
