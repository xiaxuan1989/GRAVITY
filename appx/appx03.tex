...
\begin{definition}
    给定域 $\F$。集合 $X$ 上的\textbf{函数}是指 $\F^X$ 的元素、\textbf{泛函}是指 $\F^{\F^X}$ 的元素。$\F^{\F^X}$ 中 $|X|$ 称\textbf{自由度}。
\end{definition}
...
则 $\hom(V,W)$ 成为了线性空间。
考虑对偶函子 $(\oo)^*:\cate{Vect}(\F)^\mathrm{op}\to \cate{Vect}(\F)$，$V^*=\hom(V,\F)$ 是线性空间，故称为 $V$ 的\textbf{对偶空间}。$\omega\in V^*$ 称为\textbf{对偶矢量}，由于 $\hom(V,\F)\subset\F^V$，也可称\textbf{线性泛函}。
比如，行向量空间就是列向量空间的对偶空间。线性映射 $f\in\hom(X,Y)$ 诱导出拉回 $f^*\in\hom(Y^*, X^*)$。
...
具体地说，考虑对偶的复合\footnote{当然，$(\oo)^{**}$ 的准确写法是 $D\circ D^\mathrm{op}$，这里记 $D(\oo)=(\oo)^*$。} $(\oo)^{**}:\cate{Vect}(\F)\to\cate{Vect}(\F)$，则 $V^{**}=\hom(V^*,\F)$。
为满足“天生乘法”，只需定义 $\eta_V:V\to V^{**}$ 使 $\eta_V A(\omega):=\omega(A)$，这里 $\eta_V A$ 也可记 $A^{**}$。
对任意线性 $f$，不难检查以下图表
\[\begin{tikzcd}
    V \arrow[r, "\eta_V"] \ar[d, "f"'] & V^{**} \arrow[d, "f^{**}"] \\
    W \arrow[r, "\eta_W"] & W^{**}
\end{tikzcd}\]
是交换的。可见 $\eta=\{\eta_V\}$ 就是 $\mathrm{id},(\oo)^{**}$ 的自然变换。
容易看出 $\eta_V:V\to V^{**}$ 总是单射，事实上可证 $\eta$ 是双射当且仅当 $V$ 有限维，遂有 $\eta_V\in\mathrm{Isom}(V,V^{**})$，即自然同构 $V=V^{* *}$。
...
换言之，分量就是 $g_{ij}:=g(e_i,e_j)$，$(g_{ij})$ 是 $g$ 的一个表示矩阵，也就有 $g(A, B)=g_{\mu\nu} A^\mu B^\nu$。
...
\begin{definition}
    $V$ 上一个对称、非退化的双线性实函数 g 称为 $V$ 上的一个\textbf{度规}。 $g_\flat:V\to V^*$ 称作\textbf{音乐同构}(musical isomorphism)或\textbf{典范同构}(canonical isomorphism)。度规逆 $g^{-1}:=g^{ij} e_i\otimes e_j$，或用 $\mathrm{C}_1^2(g^{-1}\otimes g)=\mathrm{id}_V$ 定义。
\end{definition}

\begin{definition}
    \textbf{升降指标}操作本质为音乐同构，故是相对于某度规而言的。比如 $(1,1)$-张量 $T$ 的降为
    $\mathrm{C}_1^1(g\otimes T)=T_\flat$。升指标用度规逆，即 $\mathrm{C}_1^1(g^{-1}\otimes T)=T^\sharp$。就矢量、对偶矢量而言，可将音乐同构称为度规对偶：$A\in V$ 的降为 $A_\flat:=g_\flat A$；$\omega\in V^*$ 的升是 $\omega^\sharp:=g^{-1}(\omega,\oo)$。
\end{definition}
