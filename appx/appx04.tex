...
Cartan 还注意到余切空间可给函数微分一个更严格的理解。标准分析学中常将 $\d f$ 定义为 $\Delta f$ 的线性主部，即 $\d f(x,\Delta x):=f'(x)\Delta x$。这里 $\Delta x$ 是从某点 $x$ 出发的位移，实际上任意大小，但物理上的理解是在 $\Delta x$ 很小时给出的微小增量。
...
\begin{definition}
    ...
    Lorentz 度规，只有一个对角元符号不同的不定度规称 Lorentz 的。对 4 维情况，号差为 $\pm 2$。按号差 $+2$ 的习惯，一般定义 $\eta_{\mu\nu}$ 为 $\diag(-1,1,1,1)$ 的矩阵元。
