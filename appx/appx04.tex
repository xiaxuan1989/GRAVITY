...
\begin{definition}
    ...
    等价说法为：$\exists$ $M$ 的开覆盖 $\topo P$ 使 $\forall O\in\topo P$，$\exists$ 同胚 $\psi:O\to V\in\topo T_u$。也可将“$V\in\topo T_u$”改成开球或 $\R^n$。
...
\end{theorem}
\begin{proof}
    因为 $\psi(x)$ 的开邻域内一定存在包含该点的开球。再注意开球与 $\R^n$ 同胚。
\end{proof}
...
\begin{definition}
    仿紧、$T_2$ 的局部欧氏空间称为\textbf{拓扑流形}，简记 $M$。将 $\R^n$ 换为 $\C^n$ 就是\textbf{复流形}。
\end{definition}

\begin{remark}
    也可用更严苛的条件：$C_2,T_2$。下一节将说明，两种定义都能使拓扑流形可度量。
\end{remark}
...
\begin{remark}
    坐标变换可显式地表为 $n$ 个 $n$ 元函数，其 $C^r$ 性由 $\R^n,\C^n$ 分析学给出。
\end{remark}
...
综上，将今后常用的 $C^\infty$-拓扑流形之定义总结如下：
\begin{quote}
    置 $T_2$ 仿紧空间 $(M,\topo T)$，若 $\exists\topo P\subset\topo T$ 使
    \begin{itemize}
        \item $M = \bigcup_{O\in\topo P}O$；
        \item $\forall O\in\topo P$，$\exists$ 同胚 $\psi:O\to V\in\topo T_u$； 
...
\begin{theorem}
    局部欧 $\im$ 局部路径连通 $\wedge$ 局部紧 $\wedge$ 局部可度量。
\end{theorem}
...
\end{theorem}
\begin{proof}
    已知路径连通 $\im$ 连通。反之，局部欧 $\wedge$ 连通 $\im$ 局部路径连通 $\wedge$ 连通 $\im$ 路径连通。
\end{proof}

\begin{theorem}
    $T_2 \wedge \text{局部紧} \im T_3$
\end{theorem}
...
\]
于是正则。
从而 $T_2 \wedge \text{局部紧}\iff T_2 \wedge (T_2 \wedge \text{局部紧}) \im T_1 \wedge \text{正则} \iff T_3$。
\end{proof}

拓扑单位分解（连续）：在 $T_2$ + 仿紧 且（通常）局部紧的条件下，能对任意开覆盖构造从属的连续单位分解（用 bump/收缩与局部紧性来做）。
光滑单位分解（对光滑流形）：仿紧 + $T_2$（再加上光滑结构）保证能做出从属于任意开覆盖的光滑单位分解。
所以在你问的流形语境里，“仿紧（单位分解）”是可以推出的。
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item $\rho_i$ 支撑在 $O_i$ 上：$\supp\rho_i\subset U_i$，$x\notin O_i$ 时 $\rho_i=0$；
        \item $\sum_i \rho_i=1$,
    \end{itemize}
    则称为 $\topo P$ 的一个\textbf{单位分解}(partition of unity)。若在 $O_i$ 上光滑，则称\textbf{光滑单位分解}。
\end{definition}

\begin{theorem}[Bump 函数引理 / 光滑 Urysohn 引理]\label{thm:smooth_urysohn_bump}
...
\begin{theorem}
    仿紧 $\wedge$ $T_2\im$ 任何开覆盖均存在从属于它的单位分解。
\end{theorem}
\begin{proof}
    设 $\topo P=\{O_\alpha\}$ 为 $M$ 的任意开覆盖。欲构造单位分解。

    \medskip
    \noindent\textbf{将开覆盖可数化并做局部有限细化。}
    $C_2$ $\im$ Lindel\"of：任意开覆盖都有可数子覆盖。
    于是存在可数指标集 $I=\mathbb N$ 及开集 $O_i\in\topo P$ 使得 $\{O_i\}_{i\in\mathbb N}$ 仍覆盖 $M$。

    进一步，由于 $M$ 是可数基的 $T_2$ 流形，所以是副紧(paracompact)的，因此每个开覆盖都有\emph{局部有限}的开细化。
    因而存在局部有限开覆盖 $\{U_i\}_{i\in\mathbb N}$，使得对每个 $i$ 都有
    \[
        U_i\subset O_i.
    \]
    ...
    由于 $M$ 局部紧且正规(normal)，对每个 $i$ 可取开集 $V_i$ 使得
    \[
...
\begin{theorem}[Smirnov]
    $\text{仿紧}\wedge T_2\wedge\text{局部可度量}\im$ 可度量
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
    定义一：$\text{仿紧} \wedge T_2 \wedge \text{局部欧}\im \text{仿紧} \wedge T_2 \wedge \text{局部可度量} \xRightarrow{\text{Smirnov}}$ 可度量。

    定义二：$C_2 \wedge T_2 \wedge \text{局部欧} \im C_2 \wedge T_3 \xRightarrow{\text{Urysohn}}$ 可度量。
\end{proof}
\begin{remark}
...
首先，线性性的确构成线性空间：可相加，可数乘，还可定义零元 $0(f)=0$。这样 $\{\p_i\}$ 的确线性无关：$A=0\Rightarrow A^i=A^j\p_j(x^i)=A(x^i)=0$，因此 $\{\p_i\}$ 就是基。
其次，乘法法则给出 $A(1)=A(1\cdot 1)=2A(1)\Rightarrow A(1)=0$，再由线性性知$\forall c\in\R,A(c)=A(c\cdot 1)=c A(1)=0$。
%%
从而 $A$ 作用于 $f$ 的确只剩下 $A(f)=A(x^\mu)\p_\mu f(p)$，高阶项为零是因为必然存在 $x^j(p)-x^j(p)=0$。故线性性、乘法法则已经能反求得 $A=A^\mu\p_\mu$。因此这就是我们所需的全部求导代数性质了。
当然，对无穷级数滥用代数运算是极为危险的，其次也不需要 $f$ 解析，故我们更倾向于考虑有限项相加。

另一个关键在于，简单谈及 $C^r(M)$ 会忽视 $x^\mu$ 这种不在 $M$ 上的函数。
为合法地谈及坐标分量 $A(x^\mu)$，应将 $U$ 上函数拓展到 $M$ 上。而只要 $f_1,f_2\in C^r(M)$ 在 $U\in\topo N(p)$ 上相等，则 $p$ 点任意切矢 $A$ 满足 $A(f_1)=A(f_2)$。对此只需置 $f:=f_1-f_2$ 并证明 $A(f)=0$。令 $h$ 在 $M\backslash U$ 上为零但 $h(p)\ne 0$，就可构造零元 $fh=0$，则 $0=A(fh)=h(p)A(f)$ 从而 $A(f)=0$。
