...
即它是两个矢量的线性函数 $g:V\times V\to\R$。首先有对称性 $g(A,B)=g(B,A)$；其次，若 $\forall B\in V$ 都有 $g(A,B)=0$，我们希望这只在 $A=0$ 时才能发生。这称为\textbf{非退化性}。规定度规的坐标分量是 $g_{ij}:=g(\del_i,\del_j)$，这样立即得出了 \eqref{eq:g'=gdd} 式。这样 $g_{ij}$ 可以看成 $g$ 在某坐标系下具体表达出的若干实数，$(g_{ij})$ 是 $g$ 的一个分量矩阵，且是对称的：
