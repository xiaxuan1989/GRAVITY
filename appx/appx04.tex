...
一组基矢量，称之为\textbf{自然坐标（切）基底}，简称自然基底；空间$T_{x_0}M$的维数是$m$。
\end{proof}
...
\begin{definition}
    ...
    曲线$\gamma(t)$切线的切矢量，简称\textbf{切线切矢量}。 
...
\begin{definition}
    切空间$T_{x_0}M$的对偶空间称为微分流形$M$在点$x_0$的\textbf{余切空间}，
    记作$T_{x_0}^{*}M$。余切空间的元素称为\textbf{余切矢量}。
\end{definition}
首先，
余切空间维数与切空间相同，都是$m$维；其次，
定义自然切基矢的\textbf{对偶自然基矢}，
...
很明确地显示了由$f$诱导的对偶矢量是其普通\textbf{微分}${\rm d}f$。
...
(3) 设$\phi:M\to N$是浸入的，若$\phi$是\CJKunderwave{整体}单一的，则称为\textbf{嵌入映射}（embed）。
...
(5) 若$m> n, {\rm rank}_p\phi=n$，则称$\phi$点$p$是\textbf{局部淹没}的（submersion）。
...
$U$上\textbf{自然标架场}，任一$U$上切矢量场$v^a(x)$都可以用此标架展开
...
\begin{definition}
    ...
    则称$v^a$是$U$上\textbf{光滑切矢量场}。
    若$v^i$对流形$M$每个坐标卡中开覆盖是$C^\infty$的，则称$v^a$是$M$上\textbf{光滑切矢量场}；$M$上
...
\begin{definition}\label{chdm:def_cotangent-vector-field}
    ...
    的一个\textbf{余切矢量\CJKunderwave{场}}。一般记为$\mathfrak{X}^{*}(M)$或者$A^1(M)$。
\end{definition}

余切矢量的拉回同样可以拓展到“场”：
\begin{definition}\label{chdm:def_pullback-1form-onfield}
    余切矢量\CJKunderwave{场}间的\textbf{拉回映射}定义为，
    \begin{equation}
        (\phi^{*}\omega)_a v^a \overset{def}{=} \omega_a (\phi_{*}v)^a ,
...
\begin{definition}\label{chdm:def_pushforward-vector-onfield}
    定义切矢量\CJKunderwave{场}的\textbf{推前映射}，或称为\textbf{切映射}（其中$\phi$是微分同胚）
...
\begin{definition}\label{chdm:def_tensor-field}
    ...
    则称$T^{a_1\cdots a_p}_{\phantom{a_1 \cdots a_p}b_1\cdots b_q}(x)$是$M$上的一个\textbf{张量\CJKunderwave{场}}。
\end{definition}
...
\begin{definition}
    ...
    是$C^\infty(U)$的，
    则称$T^{a_1\cdots a_p}_{\phantom{a_1 \cdots a_p}b_1\cdots b_q}$是$U$上\textbf{光滑张量场}。
...
\begin{definition}
    ...
    则称曲线$\gamma(t)$是切矢量场$v^a$的\textbf{积分曲线}（或\textbf{轨道}）。
\end{definition}
...
\begin{definition}\label{chdm:def_vector-complete}
    ...
    参数$t$可以取遍整个实数轴$\mathbb{R}$，那么称$v^a$是\textbf{完备}切矢量场。
\end{definition}
...
\begin{definition}\label{chdm:def_1PTG-local}
    ...
    那么，称$\phi_t$为作用在开子集$U$上的\textbf{\CJKunderwave{局部}（左）单参数可微变换群}。
\end{definition}
...
\begin{proof}
    ...
    单参数$t$便是所选坐标系的第一个坐标。
    一般称坐标系$(V;y^i)$是矢量场$X^a$的\textbf{适配坐标系}。 \index[physwords]{适配坐标系}
\end{proof}
