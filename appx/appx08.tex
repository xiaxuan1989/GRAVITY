...
\begin{definition}
    ...
    $\forall b\in B$，空间$\pi^{-1}(b)\in E$称为植于$ b\in B$点的\textbf{纤维}（fibre）。
\end{definition}
...
\begin{definition}
    若映射$s: B\to E$使得$\pi \circ s = {\rm id}$成立，则称$s$是丛$(E, B,\pi)$的\textbf{截面}。
...
\begin{definition}\label{chfb:def_vf-section}
    ...
    并且满足$\pi \circ s \equiv {\rm id} :M\to M$，则称映射$s$是矢量丛$E$的一个\textbf{截面}。
...
局部标架场$\left\{s_\alpha\right\}$下的\textbf{联络型式}
...
则称光滑截面$\xi$沿曲线$\gamma$是\textbf{平行}的；当然，此时只需要 $\xi$ 沿 $\gamma$ 有定义即可。
...
\begin{definition}
    ...
    则称此丛为$M$上以$G$为结构群的\textbf{\bfseries G-主丛}，记为 $(E, M, \pi, G)$。
\end{definition} 
...
\begin{definition}\label{chfb:def_associated}
    ...
    U_\alpha \cap U_\beta \rightarrow G\right\}$，则称这两个纤维丛是\textbf{相配的}（associated）。
\end{definition} 
...
若把纤维型限定为它的子群$O(m)$，则得到它的子丛\textbf{正交切标架丛}，
...
进而可在丛空间$E$上定义\textbf{丛度量}$\langle \cdot,\cdot\rangle_{\mathfrak{g}}$为Killing型的{\kaishu 负值}
...
\end{equation}
满足变换关系\eqref{chfb:eqn_GDG}的式\eqref{chfb:eqn_GGamma}称为矢量丛的\textbf{$\boldsymbol{G}$型联络}。
