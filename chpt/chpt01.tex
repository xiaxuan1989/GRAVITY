...
拉氏量涉及时间导数 $\del_t\Phi$、相邻微小单元间的势能 $\del_i\Phi$ 的积分：$L=\int\mathcal L(\Phi,\Phi_{,\mu})\d[3]{x}\Rightarrow S=\int \mathcal L \d[4]{x}$。$\mathcal L$ 称为\textbf{拉氏密度}。
同理，默认许多理论能存在 $\mathcal L$ 表述；$\mathcal L$ 一般不显含时空点的坐标 $x^\mu$，且\textit{至少}含一阶导。在粒子情形我们通过决定性原理排除高阶导，但现在是关于时空坐标的导数，且空间项有相互作用势的含义，冒然排除高阶是激进做法。一般对若干场（仍简写 $\Phi$）应记
\eq{
S(\Phi(x))=\int \mathcal L(\Phi,\Phi_{,\mu_1},\cdots,\Phi_{,\mu_1\cdots\mu_k})\d[4]{x}.
}
若 $\mathcal L$ 至多含二阶导，即 $\mathcal L(\Phi,\Phi_{,\mu})$，则类似计算可得场的 E-L 方程
\eq{
\pdv{x^\mu} \pdv{\mathcal L}{\Phi_{,\mu}}-\pdv{\mathcal L}{\Phi}=0.
}
当然，实操中总是直接计算变分。
...
\begin{enumerate}
    ...
    \item 考虑积分域 $U$时，规定边界上任意阶都有 $(\delta \Phi)_{,\mu_1\cdots\mu_k}=0$（比如认为 $U$ 外的旁路 $\Phi$ 恒定地与边界一致），这一更强的条件使 $\mathcal L$ 无论含多少阶导数都能消除边界项。
\end{enumerate}
...
$q(t)\to\tilde q(\tilde t)$ 下路径的改变为\textbf{等时变分} $\bar\delta q^i(t)=\tilde q^i(t)-q^i(t)$，参数的改变 $\delta t=\tilde t-t$。这里的变分统称\textbf{Noether 变分}，注意与前文区分\footnote{当然，等时变分和 Fréchet 变分的代数性质一致。}。之后将 $q^i(t)$ 简写为 $q^i$。无穷小时即 $\delta t =\epsilon \eta(t, q^i, \dot{q}^i), \bar\delta q^i =\epsilon \xi^i(t,q^i,\dot{q}^i)$。
注意 $\tilde q^i(\tilde t)=\tilde q^i+\dv{t}(\tilde q^i)\delta t+O(\epsilon^2)$，代入 $\tilde q^i=\bar\delta q^i+q^i$ 可以看到 $\dv{t}(\bar\delta q^i)\delta t$ 也是 $O(\epsilon^2)$，所以总变化保留至一阶有
\[
\delta q^i=\tilde q^i(\tilde t)- q^i=\bar\delta q^i+\dot q^i \delta t,\quad \delta\dot q^i=\dv{\tilde t}(\tilde q^i(\tilde t))-\dot q^i=\bar\delta\dot q^i+ \ddot q^i\delta t.
\]
后者是直接替换；当然，也可同理地直接计算并注意 $\bar\delta\dot q^i=\dv{t}(\bar\delta q^i)$。注意 $\d\tilde t=\dv{\tilde t}{t}\d t=\left(1+\dv{t}(\delta t)\right)\d t$，
$\delta q^i$ 引起的作用量变化为
\begin{align*} \delta S&=\int L\left(\tilde t,\tilde q^i(\tilde t),\dv{\tilde q^i(\tilde t)}{\tilde t}\right)\d{\tilde t}-\int L\d{t} =\int \left(1+\dv{t}(\delta t)\right)\left(L+\delta L\right)\d t-\int L\d{t}\\
&=\int \left(\delta L+L\dv{t}(\delta t)\right)\d t=\int \left(\pdv{L}{t}\delta t+\pdv{L}{q^i}\delta q^i+\pdv{L}{\dot q^i}\delta\dot q^i+L\dv{t}(\delta t)\right)\d t\\
...
\end{align*}
考虑准不变性，这就等价于上式积分的大括号为全导数项 $\dv{K}{t}$，即相差边界项 $\delta S=K_2-K_1$。但注意上式第二项已是全导数，而 $\delta t$ 的贡献全部收了进去，可见 $\delta t$ 并不影响准不变性，准不变性实际上只约束 $\bar\delta q^i$ 的形式。更多时候还是直接要求严格的对称性 $\delta S=0$，则 $\dv{K}{t}=0$，这就是\textbf{Noether 条件}，作为作用量对称性的判据。
...
考虑时空场 $\Phi(x)$。变换写作 $\Phi\to\tilde\Phi$ 和时空点变换\footnote{可见时空点变换（或一般的位形空间）和坐标变换十分相似。前者为新点、旧点在同一坐标系下的表示 $x\to\tilde x$，称\textbf{主动观点}（点变、坐标系不变）；后者为同一时空点在不同坐标系下的表示 $\{x\}\to\{x'\}$，称\textbf{被动观点}（点不变、坐标系变）。两种观点可以互相转换。} $x\to\tilde x$。则
\[
\delta\Phi =\bar\delta \Phi+\Phi_{,\nu}\delta x^\nu,\quad \delta\Phi_{,\nu}=\bar\delta \Phi_{,\nu} + \Phi_{,\sigma\nu}\delta x^\sigma,
\]
并注意 $\bar\delta(\Phi_{,\nu})=\del_\nu(\bar\delta \Phi)$。我们知道体元变换 $\d[4]\tilde x=J\d[4]x$，其中 $\det(\pdv{\tilde x^\mu}{x^\nu})$ 为 Jacobi 行列式。代入 $\delta x^\mu$ 则 $J=\det(\delta_\nu^\mu+\del_\nu\delta x^\mu)$。尝试继续展开至一阶。


\eq{
    J=1+\del_\mu\delta x^\mu,
}
...
\eq{
    \Delta S:=\int \mathcal L\left(\tilde x,\tilde\Phi(\tilde x),\tilde \del_\nu\tilde\Phi(\tilde x)\right)\d[4]{\tilde x}-\int \mathcal L(x,\Phi(x),\Phi_{,\nu}(x)) \d[4]{x},
}

\begin{align*} \quad~&\int  \mathcal L\left(\tilde x,\tilde\Phi(\tilde x),\tilde \del_\nu\tilde\Phi(\tilde x)\right)J\d[4]{x}  =\int \left(1+\del_\sigma\delta x^\sigma\right) \left(\mathcal L+\del_\nu \mathcal L \delta x^\nu\right. \\ &+\left.\frac{\partial \mathcal L}{\partial \Phi}\left(\delta \Phi+\Phi_{,\nu}\delta x^\nu \right)+\frac{\partial \mathcal L}{\partial \Phi_{,\mu}}\left(\del_\mu\delta \Phi+  \Phi_{,\sigma,\mu}\delta x^\sigma\right)\right)\d[4]x,
\end{align*}

\eq{\Delta S=\int\left(\left(\frac{\partial \mathcal L}{\partial \Phi}-\pdv{x^\mu} \frac{\partial \mathcal L}{\partial \Phi_{,\mu}} \right)\delta \Phi+\pdv{x^\mu}\left(\frac{\partial \mathcal L}{\partial \Phi_{,\mu}} \delta \Phi+\mathcal L \delta x^\mu\right)\right)\d[4]x.}

\eq{
    \left(\frac{\partial \mathcal L}{\partial \Phi}-\pdv{x^\mu} \frac{\partial \mathcal L}{\partial \Phi_{,\mu}} \right)\delta \Phi+\pdv{x^\mu}\left(\frac{\partial \mathcal L}{\partial \Phi_{,\mu}} \delta \Phi+\mathcal L \delta x^\mu\right)=0,
}
可见 $\delta x^\mu$ 的贡献总是时空导数的形式，故仅对 $\delta \Phi$ 做出要求。进一步地，若 $\Phi(x)$ 是正路且做整体变换时，有
\eq{
    \del_\mu J^\mu=0,\quad  J^\mu:=\frac{1}{\epsilon}\left(\frac{\partial \mathcal L}{\partial \Phi_{,\mu}} \delta \Phi+\mathcal L \delta x^\mu\right)=\frac{\partial \mathcal L}{\partial \Phi_{,\mu}} F+\mathcal L \xi^\mu.
}
这就是 \textbf{Noether 守恒流}。由于 $\mathcal L$ 包含求和的意义，因此守恒量需要其空间积分来产生。

当然，不同于粒子情形，场论的拉氏量可能涉及高阶导数，甚至是复数场，因此一般具体问题具体分析，但整套计算程序是相似的。
...
要求 $\Delta \Phi=0$，则
\eq{
    J^\mu=\frac{\partial \mathcal L}{\partial \Phi_{,\mu}} \frac{\Delta \Phi-\Phi_{,\nu}\delta x^\nu}{\epsilon}+\mathcal L \xi^\mu=-S^{\mu\nu}\xi_\nu,\quad S^{\mu\nu}:=\pdv{\mathcal L}{\Phi_{,\mu}}\Phi^{,\nu}-\mathcal L\eta^{\mu\nu}.
}
其中 $S^{\mu\nu}$ 。


然而，从定义看出 $S^{\mu\nu}$ 不一定是对称张量。我们希望能在 $S^{\mu\nu}$ 上

称为 \textbf{Belinfante 张量}

\textbf{正则}(canonical)\textbf{能动张量}
...
后者称为\textbf{4-动量}。$x^0$ 代表时间，因此在相对论中能量为
\eq{
E=P^0=\sqrt{p^2+m^2}=\gamma m=m+\frac{1}{2} mu^{2}+\cdots,
}
它也正是 $p_i u^i-L$。


易知 $p^2:=p_i p^i=m^2u^2/(1-u^2)$，则 $u={p}/{\sqrt{p^2+m^2}},\gamma={\sqrt{p^2+m^2}}/{m}$。故
...
\eq{
    \delta \Phi(x) =\epsilon F(x),
}

\eq{\delta \phi :=\tilde \phi-\phi=\i \epsilon \phi, \quad \delta \bar{\phi}=-\i \epsilon \bar{\phi},\quad\delta x^\mu=0,}
则
\eq{
    \delta \mathcal L=\del_\mu J^\mu, 
}
...
然 $\phi$ 只代表电荷系统，电磁场从何而来？

原来，场论的定域性使相位往往应随时空点可变，这称为相位变换的\textbf{局域化}或\textbf{规范化}(gauging)。

局域化一下

为得局域 U(1) 对称性，需引入矢量场 $A_\mu$，而最小耦合指出，只需给作用量加上缩并项 $\i e\left(\phi\partial^\mu \bar{\phi}-\bar{\phi} \partial^\mu \phi \right) A_\mu$，则
\eq{
