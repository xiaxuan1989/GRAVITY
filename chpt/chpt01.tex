...
\begin{align}
    \delta S&=\int_{a}^{b}\delta L\d{t}=\int_{a}^{b}\bigg(\pdv{L}{q^i}\delta q^i+\pdv{L}{\dot q^i}\delta \dot q^i\bigg) \d{t}\nonumber\\
    &=\int_{a}^{b}\left(\pdv{L}{q^i}-\dv{t}\pdv{L}{\dot q^i}\right)\delta q^i\d{t}+ \eval{\pdv{L}{\dot q^i}\delta q^i}_{a}^{b}\nonumber\\
...
\begin{align*} \delta S&=\int \left(\delta L\d t+L\delta(\d t)\right)=\int \left(\delta L+L\dv{t}(\delta t)\right)\d t\\
&=\int \left(\pdv{L}{q^i}\bar\delta q^i+\pdv{L}{\dot q^i}\bar\delta\dot q^i+L\dv{t}(\delta t)+\dot L\delta t\right)\d t\\
...
设任意（分段光滑的）观者线 $C(\sigma)$，$\sigma$ 是任取参数，可有 $\gamma=\dv*{t}{\tau}$。
考察它于事件 $C(\sigma_1),C(\sigma_2)$ 的固有时间 $\Delta\tau$。设置其固有坐标系 $\{x'\}$（$x'^0=\tau$），$C$ 上事件总满足 $x'^i=0$，则 $\d\tau^2=-\d s^2$。它在惯性系 $\{x\}$ 下的参数式记 $x^\mu(\sigma)$，则
\eq{
...
第 $\alpha$ 坐标轴可按相应 $x^\alpha$ 参数化，切矢 $\dv{x^\mu}{x^\alpha}=\delta^\mu_\alpha$ 称为此系的\textbf{坐标基矢}（在此系的分量），这里 $\delta^\mu_\nu$ 也是 Kronecker 符号。$\{\delta_\alpha^\mu\}_{\alpha=0}^3$ 称\textbf{坐标基}。可见惯性系的时空轴正交，因为 $\eta_{\mu\nu}\delta^\mu_0\delta^\nu_i=\eta_{0i}=0$。这说明我们进行对钟并发展相应的几何后，同时面与各观者线正交，换言之，时空正交性在实验上由对钟产生\footnote{几何的理论概念只能代数地定义，如勾股定理。实际用物理定律来约定一类基准，各类规则形状的日常用品就是这样生产的。比如，默认光走空间直线、Lorentz 力确定空间正交性、宇称不守恒确定空间右手性。}。
...
光速不变、狭义相对论原理等命题就应更严格地阐述为一种\textbf{不变性}(invariance)：
\begin{quote}
...
对单个自由质点构造拉氏量。其位形空间就是时空本身，世界线是路径，参数可取坐标时或固有时。
构建相对性理论的关键在于使用 Lorentz 不变的 $S$ 来获得运动方程。
我们能找的只有 $\d\tau,m,c=1$，故 $S\propto m\int \d\tau$。注意速率 $u=\sqrt{\delta_{ij}\dot{x}^i \dot{x}^j}\ll 1$ 时
\[
...
注意 $U^\mu=\gamma\dot x^\mu$，而 $\dot\gamma=\gamma^3\dot{x}_\mu\ddot{x}^\mu=\gamma^3\dot{x}_i\ddot{x}^i$（因为 $\dot x^0=1,\ddot x^0=0$），显然 $\pdv*{L}{x^\kappa}=0$，故 E-L 方程为 $\dot P_\mu = \dot m U_\mu + m\eta_{\mu\nu} (\gamma\ddot x^\nu + \gamma^3\dot{x}_i\ddot{x}^i\dot x^\nu)=0$。
给 $\dot P_\mu$ 乘上 $\gamma=\dv*{t}{\tau}$ 就相当于重参化\footnote{若想直接将作用量的参数取为固有时 $\tau$，则默认是\textit{正路}的固有时。对于任何邻近旁路，$\tau$ 都非其固有时而只是纯粹的参数。为何不能用每条路径自己的固有时来作参数？因为变分应与参数无关。比如置有限积分域，Fréchet 空间中任意邻近旁路上所用参数的取值范围，必为同一区间，若每条路径皆用自己的固有时作参数就办不到。
...
\eq{
    \dv{P_\mu}{\tau}= \dv{m}{\tau} U_\mu + m A_\mu=0,\quad A^\mu:=\dv[2]{x^\mu}{\tau}=U^\nu\p_\nu U^\mu=\gamma^2\ddot x^\mu + \gamma^4\dot{x}_i\ddot{x}^i\dot x^\mu.
}
$A^\mu$ 称\textbf{4-加速}。
假设 $\dv*{m}{\tau}=0$，约去 $m$ 并与 $\eta^{\mu\kappa}$ 缩并有 $A^\mu=0$，也即 $\ddot x^\nu + \gamma^2\dot{x}_i\ddot{x}^i\dot x^\nu=0$。
取 $\nu=0$ 可知 $\dot{x}_i\ddot{x}^i=0$，故 $A^\mu=0$ 等价于 $\ddot x^\mu=0$。
这都是直线方程，符合实验观测，可认为作用量构造正确。
我们知道两点间直线最短，但由于 $\d s^2=-\d\tau^2$，故时空直线是使固有时最大而非最小，即长程线\footnote{对于分段光滑情形，显然总能转化为分段直线情形，故只需考虑三角情形，而显然斜线的固有时短于竖线，故结论仍成立。曲线性质更差的情形见第 \ref{chpt:causal} 章。}。
...
由时空对称性，$P^\mu$ 分量分别解释为\textbf{3-动量} $P^i=\gamma m \dot{x}^i$ 和能量 $E=P^0=P_i \dot{x}^i-L=\gamma m$。
...
同理，考虑 Hamilton 主函数 $S(x^\mu)$，通过在壳变分 $x^\mu\to x^\mu+\delta x^\mu$ 可分析出 $P_\mu=\pdv*{S}{x^\mu}$，从而得到 Hamilton-Jacobi 方程 $\eta^{\mu\nu}\pdv{S}{x^\mu}\pdv{S}{x^\nu}=-m^2$。
...
目前未发现超光速的快子(tachyon)，故我们更愿意承认亚光速粒子、光子在时间上的连续性，即确认某两事件属同一粒子。

凡信息就需载体，故光速是信息传速上限。注意，存在粒子能依次经历的两事件才称有\textbf{因果联系}，可见并非所有事件都能涉及因果。
...
\end{enumerate}
$U^\mu$、$U_\mu$、Kronecker 符号\footnote{注意 $\{x\}$ 的第 $x^0$ 坐标基在 $\{x\}$ 下表为 $\delta^\mu_0$，所以变换到另一系 $\{x'\}$ 为 $\Lambda^{\mu}{}_{\nu}\delta^\nu_0=\Lambda^{\mu}{}_{0}$。}都是 Lorentz 张量。偏导保持 Lorentz 协变，故 4-加速 $U^\nu\p_\nu U^\mu$ 也是 Lorentz 张量。
...
从而一阶意义上 $U^\mu=\delta^\mu_0$。此时运动方程为 $\dv*{U^\mu}{\tau}=-\Gamma_{00}^\mu= \frac 12 g^{\mu\nu} g_{00,\nu}=\frac 12 \eta^{\mu\nu} h_{00,\nu}$，则 $-\p^i\phi=\dv*[2]{x^i}{t}=\frac 12 \delta^{ij} h_{00,j}$，故
\eq{
...
切矢 $T^\mu=\dv*{x^\mu}{\sigma}$ 沿自身平行移动的曲线称为\textbf{测地线}(geodesic)，即 $T^\mu\cd_\mu T^\lambda =0$。
...
对超曲面的任意切矢 $w^\mu$ 满足 $n_\mu w^\mu = 0$ 的背景余切矢 $n_\mu$ 称为\textbf{法余矢}，$n^\mu = g^{\mu\nu}n_\nu$ 称为\textbf{法矢}。总可取坐标系使某个 $\hat x^0=f$ 坐标线即为超曲面，换言之超曲面方程为 $f(x^\mu)=C$ 或 $\d f=0$，其梯度 $\p^\mu f$ 就是法矢，因为对任意曲面切矢 $T^\mu$ 总可在曲面上找到 $x^\mu(\lambda)$ 使 $T^\mu=\dv*{x^\mu}{\lambda}$，进而 $T^\mu\p_\mu f=\dv*{f}{\lambda}=0$。任意余切矢分解为 $V_\mu = A \p_\mu f + B T_\mu$，则 $V_\mu w^\mu = B T_\mu w^\mu$。故法余矢必如 $A \p_\mu f$，即只相差倍数关系，可约定内外。
...
可验证 $h_{\mu\nu}$ 就是一个诱导度规。假设选取坐标系使得超曲面为坐标线：$n^\mu = \delta^\mu_0$，单位性即 $g_{00}=\pm 1$，故 $n_\mu = g_{\mu 0}$，故 $h_{0\nu} = g_{0\nu}(1\mp g_{0 0})=0$。换言之，用该系表示度规为 $\d s^2=\pm(\d x^0)^2+h_{ij}\d x^i\d x^j$，只需令坐标 $x^0$ 为常数就得到了 $h_{\mu\nu}$。此系下也有 $g=\pm h,h=\det(h_{ij})$。比如，先用球坐标 $\{r,\theta,\phi\}$ 改写欧氏度规为
...
\subsection{曲率及其性质}\label{sec:curv}
