...
于 $(t_0,r_0)$ 释放自由落体质点 $Z^\mu$，观测结果建议其内向。E-L 方程给出 $\dv{\tau}(g_{\mu\nu} Z^\nu)=\frac 12 g_{\alpha\beta,\mu} Z^\alpha Z^\beta$。由 $g_{\mu\nu,0}=0$ 知 $g_{0\nu} Z^\nu=\text{常数}$。取 $\nu=0$ 有 $Z^0=\sqrt{1-2M/r_0}/(1-{2M}/{r})$，因为 $r=r_0$ 时 $Z^0=1/\sqrt{1-2M/r_0}$。
由模长归一性并取负有 $Z^1=-\sqrt{2M/{r}-2M/{r_0}}$。
下面按自由落体者建立新系 $\{\tau,R\}$。$\tau$ 可称\textbf{自由落体时}。
由 $\tilde Z^\mu=\delta^\mu_0$ 知 $\tilde g_{0 0}=\tilde g_{\mu\nu}\tilde Z^\mu\tilde Z^\nu=-1$ 且对偶为 $\tilde Z_\mu = \tilde g_{\mu0}$。设 $\tilde g_{01} = 0$ 表示已对钟。代入 $\pdv{\tilde x^\mu}{x^\nu}\tilde Z_\mu=Z_\nu$ 得 $\pdv{\tau}{t}=-Z_0,\pdv{\tau}{r}=-Z_1$，
即
\eq{
    \d\tau=-Z_0\d t-Z_1\d r=\sqrt{1-\frac{2M}{r_0}}\d t+\frac{\sqrt{2M/{r}-2M/{r_0}}}{1-{2M}/{r}}\d r.
}
做 $t\to\tau$ 变换后有
\[\d s^2=-\d\tau^2+\left(\frac{-Z^1\d\tau+\d r}{Z_0}\right)^2+r^2\d\Omega^2,\]
其中加上了没动过的空间部分。
...
直接代入上述度规可得
\eq{
    \d s^2 = -\d\tau^2+\frac{1}{A^2}\left(\frac{2M}{r}-\frac{2M}{r_0}\right)\d R^2+r^2\d\Omega^2,
}
...
即
\eq{
    \d\tau=\d t+\frac{\sqrt{2M/r}}{1-{2M}/{r}}\d r\implies\gets \tau = t+2 \sqrt{2Mr}+2M\ln\abs{\frac{\sqrt{r}-\sqrt{2M}}{\sqrt{r}+\sqrt{2M}}}.
}
\eq{\d s^2=-\d\tau^2+\left(\sqrt{\frac{2M}{r}}\d\tau+\d r\right)^2,}
这称为\textbf{Gullstrand–Painlevé 系}
$A=1$，则 $R=\tau+\frac 23(2M)^{-1/2}r^{3/2}$。
度规表为
\eq{
