...
按这种方式变换的矢量则称\textbf{协变矢量}(covector)。这一称呼与“协变性”有区别，没有物理方程“逆变性”的说法。推而广之可得\textbf{Lorentz 张量}，满足 Lorentz 变换下的张量变换律：
\eq{
...
一点处全体逆变矢量构成该点\textbf{切空间}(tangent space)。物理学很多矢量都与相切有关，因而是逆变矢量。

可以存在变换与之互逆的矢量：
\eq{\omega'_i=\pdv{x^j}{x'^i}  \omega_j,}


或\textbf{余切矢}(cotangent vector)，简称\textbf{余矢}。下指标称\textbf{协变指标}。
...
度规是一个\textit{指定好的}对称、非退化 2-协变张量
...
我们的确期望的是，如果有分量，就应该有某种数学对象，使得分量是这一数学对象在不同坐标系下的不同转述。当然希望能用类似 $A=A^I e_I$ 的形式表述矢量。换一组 $\{e_I\}$ 就可给出另一组分量，但这又涉及 $e_I$ 的定义是什么。鉴于对切矢图像的充分观察，Cartan 率先给出了这一数学对象的构造，称为\textbf{映射语言}，详见附录 \ref{appx:manifold}。但这套框架比较抽象。很多人因其语言之优雅，成了 Cartan 及其后世之信徒，常声称其优势在于“无需借助坐标”，即 Cartan 关于矢量的定义是“几何”的、“坐标无关”的。这一论断当然过分夸张。
