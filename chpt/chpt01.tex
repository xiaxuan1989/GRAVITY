...
其中注意矩阵转置满足 $(R^\mathrm{T})_k{}^i=R^i{}_k$；矩阵乘法顺序涉及指标的相靠。从正交右手归一系 $\{x\}$ 出发，只要坐标变换是保定向的，即 Jacobi 行列式满足 $J=\det(\pdv{x'^i}{x^k})>0$，则称新系 $\{x'\}$ 也是右手系。对上式取行列式有  $|R|^2=1$。当 $|R|=1$ 时它就是旋转变换，当 $|R|=-1$ 时就存在空间反射。右手 Descartes 系间相差保定向旋转变换、原点平移，它们都保持 $\d\l$ 不变。
...
\begin{enumerate}
    ...
    \item \textbf{坐标偏导} $T^{\cdots}{}_{\cdots,\mu}=\del_\mu T^{\cdots}{}_{\cdots}=\pdv*{T^{\cdots}{}_{\cdots}}{x^\mu}$ 保持 Lorentz 协变。
\end{enumerate}
...
拉氏量涉及时间导数 $\del_t\Phi$、相邻微小单元间的势能 $\del_i\Phi$ 的积分：$L=\int\mathcal L(\Phi,\Phi_{,\mu})\d[3]{x}\Rightarrow S=\int \mathcal L \d[4]{x}$。$\mathcal L$ 称\textbf{拉氏密度}。
对若干场 $a=1,\cdots,n$ 一般地有
\eq{
S(\Phi_a(x))=\int \mathcal L(x^\mu,\Phi_a,\Phi_{a,\mu})\d[4]{x}\implies \pdv{x^\mu} \pdv{\mathcal L}{\Phi_{a,\mu}}-\pdv{\mathcal L}{\Phi_a}=0.
}
这里\textit{坐标系可以是任意的}。
当然，作为物理规律的 $\mathcal L$ 一般不显含 $x^\mu$。在粒子情形我们通过决定性原理排除高阶导，由于现在 $\del_\mu$ 还包含相互作用势的含义，不能冒然排除高阶：事实上 $\mathcal L$ 的确可能存在高阶导 $\Phi_{,\cdots}$，则 E-L 方程一般需要修改；但若高阶导只是表面上看起来存在，实际可通过某种方式消除（马上将看到），则 E-L 方程照旧。实操中总是直接计算变分，即可规避“E-L 方程可能错误”这种麻烦。
...
\begin{enumerate}
    ...
    \item 仅在 $\delta \Phi=0$ 条件下，可通过修改 $\mathcal L$ 使其消除边界项；
    \item 考虑积分域 $U$时，规定边界上任意阶都有 $(\delta \Phi)_{,\cdots}=0$（比如认为 $U$ 外的旁路 $\Phi$ 恒定地与边界一致），这一更强的条件使 $\mathcal L$ 无论含多少阶导数都能消除边界项。
\end{enumerate}
...
\[
\delta\Phi =\bar\delta \Phi+\Phi_{,\nu}\delta x^\nu,\quad \delta\Phi_{,\nu}=\bar\delta \Phi_{,\nu} + \Phi_{,\sigma\nu}\delta x^\sigma,\quad \delta\mathcal L =\bar\delta\mathcal L+\mathcal L_{,\nu}\delta x^\nu,
\]
...
代入 $\delta x^\mu$ 则 $\pdv{\tilde x^\mu}{x^\nu}=\delta_\nu^\mu+\del_\nu\delta x^\mu$。零阶项 $\varepsilon_{\mu_0\cdots \mu_3} \delta_0^{\mu_0}\cdots\delta_3^{\mu_3}=\varepsilon_{0\cdots 3}=1$。一阶项为 $\varepsilon_{\mu_0\cdots \mu_3}\delta_0^{\mu_0}\cdots\del_\nu\delta x^{\mu_\nu}\cdots\delta_3^{\mu_3}=\delta_{\mu_\nu}^{\nu}\del_\nu\delta x^{\mu_\nu}=\del_\nu\delta x^\nu$。所以 $J=1+\del_\mu\delta x^\mu$，也即 $\delta(\d[4]{x})=\del_\mu\delta x^\mu\d[4]{x}$。综上
\begin{align*} \delta S
&=\int \left(\delta\mathcal L+\mathcal L \del_\mu\delta x^\mu\right)\d[4]{x}=\int \left(\bar\delta\mathcal L+\del_\mu(\mathcal L\delta x^\mu)\right)\d[4]{x}\\
&=\int \left(\left(\pdv{L}{\Phi}-\pdv{x^\mu} \pdv{\mathcal L}{\Phi_{,\mu}}\right)\bar\delta\Phi+\pdv{x^\mu}(\pdv{\mathcal L}{\Phi_{,\mu}}\bar\delta\Phi+\mathcal L\delta x^\mu)\right)\d[4]{x}.
\end{align*}
若 $\Phi(x)$ 是正路且做整体变换时，有
\eq{
    \del_\mu J^\mu=0,\quad  J^\mu:=\frac{1}{\epsilon}\left(\pdv{\mathcal L}{\Phi_{,\mu}}\bar\delta\Phi+\mathcal L\delta x^\mu\right)=\pdv{\mathcal L}{\Phi_{,\mu}} F+\mathcal L \xi^\mu.
}
这就是 \textbf{Noether 守恒流}。含有高阶导 $\Phi_{,\cdots}$ 时，表达式会有所不同。因此一般具体问题具体分析，但整套计算程序是相似的。
定义 $\rho:=J^0,j^i:=J^i$，则 $\del_t\rho+\div\bm j=0$ 就是荷 $Q=\int \rho \d[3]x$ 的连续性方程，$\rho,\bm j$ 分别解释为该系所测\textbf{荷密度}和\textbf{流密度}。
...
\eq{
    J^\mu=-S^{\mu\nu}\xi_\nu,\quad S^{\mu\nu}:=\pdv{\mathcal L}{\Phi_{,\mu}}\del^\nu\Phi-\mathcal L\eta^{\mu\nu}.
}
若考虑时空均匀的平移对称（惯性系且 $\del_\nu\xi^\mu=0$），则
\eq{
    \del_\mu S^{\mu\nu}=\del_t S^{0\nu}+\del_i S^{i\nu}=0,
}
考虑时间变换 $\xi^0$ 就是取 $\nu=0$，考虑空间变换 $\xi^i$ 就是取 $\nu=i$。
...
\eq{
    \mathcal H=S^{00}=\pi\dot\Phi-\mathcal L,\quad \pi = \pdv{\mathcal L}{\dot\Phi},
}
其中 $\pi$ 为\textbf{共轭动量密度}，$\dot\Phi=\del_t\Phi$。$S^{0i}=\pi\delta^i\Phi$ 就是\textbf{动量密度}。
分析能量连续性 $\dot H=-\int\del_i S^{i0}\d[3]x$，故 $S^{i0}$ 是\textbf{能流密度}。
...
\eq{
    g_{\mu\nu}(\tilde x)=g_{\mu\nu}(x)+\epsilon\xi^\lambda\del_\lambda g_{\mu\nu}(x),\quad \frac{\partial \tilde x^{ \rho}}{\partial x^\mu}=\delta^\rho_\mu+\epsilon\del_\mu\xi^\rho,
}
如果仿照等度规变换那样，满足
\eq{\tilde g_{\mu\nu}(\tilde x)=g_{\mu\nu}(\tilde x)\Longleftrightarrow g_{\mu \nu}(x)=\frac{\partial \tilde x^{ \rho}}{\partial x^\mu} \frac{\partial \tilde x^{ \sigma}}{\partial x^\nu} g_{\rho \sigma}(\tilde x),}
保留至一阶（即取 $\epsilon\to 0$）：
\eq{0=\xi^\lambda \del_\lambda g_{\mu\nu}+\del_\mu \xi^\rho g_{\rho \nu}+\del_\nu\xi^\sigma g_{\mu\sigma}=-2\xi_\lambda \Gamma^\lambda_{\mu\nu}+\del_{(\mu}\xi_{\nu)} =\nabla_{(\mu}\xi_{\nu)},}
...
考虑最简单的实标量场 $\phi$。置闵氏时空惯性系。场是粒子系统连续化，作用量中的动能项显然用 $\del_\mu\phi$ 构造。
最简单的选择是 $\del^\mu\phi\del_\mu\phi$。其系数总可适当标度，但要求为正，则系数为负。我们可取
\[\mathcal L= -\frac 12\del^\mu\phi\del_\mu\phi-U(\phi)\implies \square\phi-\pdv*{U}{\phi}=0,\]
从 E-L 方程可以看出，$\phi$ 的一次项对运动方程无实质贡献，故势能项最简单的取法是至少二次项，如 $U(\phi)=\frac 12 m^2\phi^2$，则得\textbf{Klein-Gordon 方程} $(\square-m^2\phi)=0$。这里 $\square=\del^\mu\del_\mu=\eta^{\mu\nu}\partial_{\mu}\partial_{\nu}$即电动力学中的 d'Alembert 算子，类似于 3 维空间中的 Laplace 算子 $\nabla^2$。
...
\eq{
    \delta \phi =\e^{\i \theta} \phi-\phi=\i e\epsilon \phi, \quad \delta \bar{\phi}=-\i e\epsilon \bar{\phi}\implies J^\mu=\i e \phi\overleftrightarrow{\partial^\mu}\bar\phi:=\i e(\phi\partial^\mu \bar{\phi}-\bar{\phi} \partial^\mu \phi).
}
...
现实里，场论的定域性使相位往往应随时空点可变，即 $\phi \to \e^{\i e\alpha(x)} \phi$，这称为变换的\textbf{局域化}或\textbf{规范化}(gauging)。原来的作用量满足整体对称性，但不满足局域对称性，因为 $\del_\mu \e^{\i e\alpha}=\e^{\i e\alpha}\i e\del_\mu\alpha\ne 0$。
为满足局域对称性，需要引入耦合项来改造 $\del_\mu$，使得新导数满足 $D_\mu\phi\to\e^{\i e\alpha(x)}D_\mu\phi$。最小耦合指出，只需以相加的方式引入：
\eq{
    D_\mu := \partial_\mu -\i e A_\mu,\quad \mathcal L=-D^\mu\phi \overline{D_\mu \phi} - U(\phi\bar\phi),
}
注意 $\overline{D_\mu \phi}=\partial_\mu \bar\phi+\i e A_\mu \bar\phi$。拆开可发现这相当于加上缩并项 $\i e \phi\overleftrightarrow{\partial^\mu}\bar\phi A_\mu$。$D_\mu$ 称为\textbf{规范协变导数}。
这个新矢量场 $A_\mu$ 称为\textbf{规范场}，$D_\mu\phi$ 的变换要求
\eq{
    A_\mu \to A_\mu +\frac{1}{e}\del_\mu \alpha(x).
}
...
为研究 $A_\mu$ 的动力学，需构造自由电磁场作用量。因为 $A^\mu$ 的运动方程应至少二阶，作用量中需要出现 $\del_\mu A_\nu$。
$A^\mu A_\mu$ 并不规范不变，但对于一阶导，可立即看到
\eq{
    F_{\mu \nu}:=2\del_{[\mu} A_{\nu]}
}
是规范不变的，因为 $F_{\mu\nu}=2\del_{[\mu}A_{\nu]} + 2\del_{[\mu} \del_{\nu]}f=\del_{\mu} (A_{\nu}+\del_\nu f)-\del_{\nu} (A_{\mu}+\del_\mu f)$。这称为\textbf{电磁张量}。它\textit{确有}场强意义：可将 $F_{i0}$ 解释为电场 $E_i$，再注意 $A_0=-\varphi,A_i=a_j$，则 $\bm E=-\grad\varphi-\del_t{\bm a}$ 就是电动力学的关系。数值上 $F^{i0}=\eta^{i\mu}\eta^{0\nu}F_{\mu\nu}=-F_{i0}$，$E^i=\delta^{ij}E_j=E_i$，所以还可写成 $F^{i0}=E^i$。
表达磁场需要引入叉乘。
...
为 $B_i=\frac 12 \varepsilon_{ijk}F^{jk}=\varepsilon_{ijk}\del^{[j}A^{k]}=\varepsilon_{ijk}\del^j A^k$，这正是 $\bm B:=\curl\bm a$。
我们总结为
\eq{
...
这种 $A^\mu$ 总是存在的，因为对任意 $\del^\mu A_\mu \ne 0$ 的 $A^\mu$，总可构造 $\tilde A_\mu = A_\mu + \del_\mu f$ 且 $\square f=\del^\mu\del_\mu f=-\del^\mu A_\mu $，即可有 $\del^\mu \tilde A_\mu =0$。“$\square f =$\,常数”的非零解存在且甚多。
...
数学上称满足 $D V^\mu/\d\tau=0$ 的 $V^\mu$ 在做\textbf{平行移动}(parallel transport)或\textbf{Levi-Civita 移动}。
...
适配性的意义在于保持内积，比如 $A^\mu U_\mu=\frac 12\frac{D}{\d\tau} (g_{\mu\nu} U^\nu U^\mu)=0$；假设 $V^\mu,W^\nu$ 沿某条线平行移动，则内积沿线恒定：$\frac{D}{\d\tau}(V^\mu W_\mu)=0$，就单个 $V^\mu$ 而言说明模方恒定。

切矢 $T^\mu=\dv*{x^\mu}{\sigma}$ 沿自身平行移动的曲线称为\textbf{测地线}(geodesic)，即 $T^\mu\nabla_\mu T^\lambda =0$。
...
假设坐标变换保持右手性，则
\eq{
    \sgn g=-1\implies 1/\tilde J=\sqrt{-g}\implies S=S_G+S_M=\int \mathcal L \sqrt{-g}\d[4]{x},
}
...
\begin{align}
    \frac{D_F V^\mu}{\d\tau} &:= h^\mu{}_\lambda Z^\sigma\nabla_\sigma (h^{\lambda\nu} V_\nu) -Z^\mu Z_\lambda Z^\sigma\nabla_\sigma ( -Z^\lambda Z^\nu V_\nu) \nonumber \\
    &= Z^\nu\nabla_\nu V^\mu + \left(h^\mu{}_\lambda (2 Z^{(\lambda} A^{\nu)}) -Z^\mu Z_\lambda (-2 Z^{(\lambda} A^{\nu)})\right)V_\nu\nonumber \\
    &= Z^\nu\nabla_\nu V^\mu + (A^\mu Z^\nu - Z^\mu A^\nu)V_\nu,
\end{align}
其中注意 $Z^\sigma\nabla_\sigma h^{\mu\nu} = 2 Z^{(\mu} A^{\nu)}$。我们看到它与协变导数相差了 $2 A^{[\mu} Z^{\nu]}$，因而有望成为一种联络。换言之，利用导数的线性性、Leibniz 法则、$\frac{D_F f}{\d\tau}=\dv{f}{\tau}$，通过缩并可将其推广到任意张量。这称为\textbf{Fermi-Walker 导数}。相应平行移动称\textbf{Fermi-Walker 移动}，简称费移。
...
只考虑引力作用，则物质走类时测地线汇，从而可研究引力潮汐。$A^\mu=0$ 使 Fermi-Walker 导数与协变导数无异，$Z^\sigma\nabla_\sigma h^{\mu\nu}=0$ 且 $Z^\nu\nabla_\nu \eta^\mu = \eta^\nu\nabla_\nu Z^\mu$。从而潮汐加速度 $a^\mu = h^\mu{}_\lambda\frac{D^2}{\d\tau^2}S^\lambda$，故可直接计算
\begin{align}
...
最小耦合指出可取 $\mathcal L_G=R$。下面计算变分。为简便运算，选择对 $g^{\mu\nu}$ 而非 $g_{\mu\nu}$变分，则 $\var(R\sqrt{-g})$ 即涉及 $g^{\mu\nu},R_{\mu\nu},\sqrt{-g}$ 三个部分。
...
\begin{align}
    \tilde t=t+2M\ln\abs{\frac{r}{2M}-1}\implies \d s^2&=-\left(1-\frac{2 M}{r}\right) {\d \tilde t}^2+\frac{4M}{r}\d\tilde t\d r+\left(1+\frac{2 M}{r}\right)\d r^2\nonumber \\
...
同理有
\eq{
 Z^\kappa \nabla_\kappa \omega_{\alpha\beta} = -\frac{2}{3}\theta \omega_{\mu\nu} - 2 \sigma_{\alpha[\nu} \omega_{\mu]}{}^{\alpha}.
}
