...
基本运算性质如下：
\begin{enumerate}
    \item 张量的直接拼凑称\textbf{张量积}\footnote{电动力学、量子力学中的\textbf{并矢}(dyadic)就是张量积，如 $\bm A\bm B,|\Phi\rangle\langle\phi|$ 等。}，结果仍为张量，比如将 $T^\mu,S_{\lambda\sigma}$ 拼成 $T^\mu S_{\lambda\sigma}$，变换式无非将各自 Jacobi 矩阵元按实数规律相乘；
...
当然，作为物理规律的 $\mathcal L$ 一般不显含时空点坐标 $x^\mu$。在粒子情形我们通过决定性原理排除高阶导，由于现在 $\del_\mu$ 还包含相互作用势的含义，不能冒然排除高阶：事实上 $\mathcal L$ 的确可能存在高阶导 $\Phi_{,\cdots}$，则 E-L 方程一般需要修改；但若高阶导只是表面上看起来存在，实际可通过某种方式消除（马上将看到），则 E-L 方程照旧。实操中总是直接计算变分，即可规避“E-L 方程可能错误”这种麻烦。
