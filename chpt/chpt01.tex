...
可见我们能按 $\Lambda^0{}_0,|\Lambda|$ 的符号将所有满足 \eqref{eq:Lorentz} 的 $\Lambda$ 分为四类。$\Lambda^0{}_0$ 的符号代表时间是否反演，因此 $\Lambda^0{}_0\geqslant 1$ 的 $\Lambda$ 称\textbf{正时的}。只有正时的变换才保持时间轴的未来、过去方向。$|\Lambda|$ 的符号代表整个时空坐标系的右手性，$|\Lambda|=1$ 时称\textbf{固有的}(proper)。进而可分为如表 \ref{table:class} 所示的四种情形。
...
注意 $U^\mu=\gamma\dot x^\mu$，而 $\dot\gamma=\gamma^3\dot{x}_\mu\ddot{x}^\mu=\gamma^3\dot{x}_i\ddot{x}^i$（因为 $\dot x^0=1,\ddot x^0=0$），显然 $\pdv*{L}{x^\kappa}=0$，故 E-L 方程为
\eq{\dot P_\mu = m\eta_{\mu\nu} (\gamma\ddot x^\nu + \dot\gamma\dot x^\nu)=m\eta_{\mu\nu} (\gamma\ddot x^\nu + \gamma^3\dot{x}_i\ddot{x}^i\dot x^\nu)=0.}
约去 $m$ 并与 $\eta^{\mu\kappa}$ 缩并有 $\gamma\ddot x^\nu + \gamma^3\dot{x}_i\ddot{x}^i\dot x^\nu=0$。
取 $\nu=0$ 可知 $\dot{x}_i\ddot{x}^i=0$，故上式即 $\ddot x^\mu=0$。这正是直线方程，符合实验观测，可认为作用量构造正确。
...
\eq{
    \dv{P_\mu}{\tau}= m\eta_{\mu\nu} (\gamma^2\ddot x^\nu + \gamma^4\dot{x}_i\ddot{x}^i\dot x^\nu) = m A_\mu,\quad A^\mu:=\dv[2]{x^\mu}{\tau}=U^\nu\p_\nu U^\mu.
}
这称为\textbf{4-加速}。故 $\ddot x^\mu=0$ 等价为 $A^\mu=0$。
...
可见若 $u\ll 1$，则 3-动量近似于经典动量 $m \dot{x}^i$，即 3-动量给出了其相对论修正。$E=\gamma m$ 即我们悉知的\textbf{质能方程}，经典动能 $(1/2)mu^{2}$ 的出现表明 $(\gamma-1)m$ 是其相对论修正。可见质点静止时也有能量，其应能释放并得到应用，对核能的开发已验证了这一点。
...
同一个 4-速在另一惯性系下测得 $U'^\mu=\d x'^\mu/\d\tau$（Lorentz 协变性），故
\eq{
    U'^\mu=\Lambda^\mu{}_\nu U^\nu.
}
...
考虑最简单的实标量场 $\phi$。置闵氏时空惯性系。场是粒子系统连续化，作用量中的动能项显然用 $\p_\mu\phi$ 构造。
...
上式在整体的\textbf{相位变换}或\textbf{U(1)变换} $\phi \to \e^{\i \theta} \phi$ 下保持不变。相应地 $\bar{\phi} \to \e^{-\i \theta} \bar{\phi}$。这就是一种内部对称性，称为\textbf{U(1) 整体对称性}。引入耦合强度 $e$ 使 $\theta=e\alpha$，取小量 $\alpha=\epsilon$，则一阶改变为（不区分 $\bar\delta,\delta$）
\eq{
...
现实里，场论的定域性使相位往往应随时空点可变，即 $\phi \to \e^{\i e\alpha(x)} \phi$，这称为变换的\textbf{局域化}或\textbf{规范化}(gauging)。原来的作用量满足整体对称性，但不满足局域对称性，因为 $\p_\mu \e^{\i e\alpha}=\e^{\i e\alpha}\i e\p_\mu\alpha\ne 0$。
为满足局域对称性，需要引入耦合项来改造 $\p_\mu$，使得新导数满足 $\D_\mu\phi\to\e^{\i e\alpha(x)}\D_\mu\phi$。最小耦合指出，只需以相加的方式引入：
\eq{
