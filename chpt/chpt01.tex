...
考虑闵氏时空上实标量场 $\phi$ 的案例。置惯性系。场是粒子系统连续化，作用量中的动能项显然用 $\del_\mu\phi$ 构造。
...
\[
J=\det(\pdv{\tilde x^\mu}{x^\nu})=\sum_{(\mu_0\cdots \mu_3)\in S_n}\sgn(\mu_0\cdots \mu_3)\pdv{\tilde x^{\mu_0}}{x^0}\cdots\pdv{\tilde x^{\mu_3}}{x^3}.
\]
代入 $\delta x^\mu$ 则 $\pdv{\tilde x^\mu}{x^\nu}=\delta_\nu^\mu+\del_\nu\delta x^\mu$。零阶项 $\sum \sgn(\mu_0\cdots \mu_3) \delta_0^{\mu_0}\cdots\delta_3^{\mu_3}=\sgn(0123)=1$。一阶项为 $\sum \sgn(\mu_0\cdots \mu_3)\delta_0^{\mu_0}\cdots\del_\nu\delta x^{\mu_\nu}\cdots\delta_3^{\mu_3}=\delta_{\mu_\nu}^{\nu}\del_\nu\delta x^{\mu_\nu}=\del_\nu\delta x^\nu$。所以 $J=1+\del_\mu\delta x^\mu$，也即 $\delta(\d[4]{x})=\del_\mu\delta x^\mu\d[4]{x}$。综上
\begin{align*} \delta S
...
考虑时间变换 $\xi^0$ 就是取 $\nu=0$，考虑空间变换 $\xi^i$ 就是取 $\nu=i$。上式就解释为 4-动量的连续性方程。所以 $S^{\mu\nu}$ 称为\textbf{正则能动张量}(canonical energy-momentum tensor)。总 4-动量为
\eq{
...
将 $\gamma$ 按二项式定理展开：
\eq{
p^i=\gamma m u^i=m u^i+\frac{1}{2} m u^{2}u^i+\cdots,\quad E=\gamma m=m+\frac{1}{2} mu^{2}+\cdots,
}
可见若 $u\ll 1$，则 3-动量近似于经典动量 $m u^i$，即 3-动量给出了其相对论修正；后者即我们悉知的\textbf{质能方程}，经典动能 $(1/2)mu^{2}$ 的出现表明 $(\gamma-1)m$ 是其相对论修正。可见质点静止时也有能量，其应能释放并得到应用，对核能的开发已验证了这一点。
...
我们曾从电荷守恒假设、若干实验定律推测出 Maxwell 方程，其优美的对称性启发我们寻找这些假设的根源。比如，量子力学的成功启发我们考虑一个复标量场 $\phi$，则作用量可用其共轭 $\bar\phi$ 构造：
\eq{
...
表达磁场需要引入叉乘。
叉乘 $\bm A\times \bm B$ 的分量写法为\footnote{$\varepsilon_{ijk}$ 称为\textbf{Levi-Civita 符号}，若 $(ijk)\in S_n$，则 $\varepsilon_{ijk}=\sgn(ijk)$，否则为零。且可证明（见附录 \ref{sec:tensor-density} 节）
\[\varepsilon_{kij} \varepsilon^{kmn} =2\delta_{[i}^{m}\delta_{j]}^{n}=2\delta_{i}^{[m}\delta_{j}^{n]}.\]
不同于普通代数等式有乘除消项的说法，指标运算中，必须理解为是用缩并得到 $\delta^\mu_\nu$ 以消项。} $\varepsilon_{ijk} A^j B^k$。这样，给定 $F^{ij}=\varepsilon^{ijk}B_k$，可以转写为 $B_i=\frac 12 \varepsilon_{ijk}F^{jk}=\varepsilon_{ijk}\del^{[j}A^{k]}=\varepsilon_{ijk}\del^j A^k$，这正是 $\bm B:=\curl\bm a$。
我们总结为
\eq{
