...
Jacobi 场固然无穷多种，但我们感兴趣满足对钟的 Jacobi 场。实际上，任意 $S^\lambda$ 总可投影为正交 Jacobi 场
\eq{\eta^\lambda=S^\lambda+Z_\nu S^\nu Z^\lambda,}
而只要初始横向曲线与线汇正交，则此后都保持正交：$Z^\nu \nabla_\nu\left(S^\mu Z_\mu\right) =Z_\mu Z^\nu \nabla_\nu S^\mu=Z_\mu S^\nu \nabla_\nu Z^\mu=\frac{1}{2} S^\nu \nabla_\nu\left(Z_\mu Z^\mu\right)=0$。测地系下 $\tilde Z^\mu=\delta^\mu_0$，知 $\tilde g_{0 0}=\tilde g_{\mu\nu}\tilde Z^\mu\tilde Z^\nu=-1$ 且对偶为 $\tilde Z_\mu = \tilde g_{\mu0}$。经初态对钟后坐标线 $\d\tau = 0$ 与测地线汇将保持正交，因此必有 $\tilde g_{0i} = 0$。
