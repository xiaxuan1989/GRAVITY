...
\eq{S(q^i(\sigma))=\int_{a}^{b}L(\sigma,q^i,\dot q^i)\d{\sigma},\quad \dot q^i:=\dv{q^i}{\sigma}.}
...
但一般不再研究 $L(\sigma,q,\dot q,\ddot q,\cdots)$，因为我们承认\textbf{Newton 决定性原理}：\textit{仅由位置和速度即可确定经典运动}，更高阶导数不会出现。

此外还可承认，作为物理规律的 $L$ 不显含 $\sigma$，也即 $\pdv*{L}{\sigma}=0$，写作 $L(q^i,\dot q^i)$。
$L$ 只通过 $q,\dot q$ 的关系隐含 $\sigma$。按理来说有 $q(\sigma)$，$L$ 就能用 $\sigma$ 表示，似乎 $L$ 就会“随参数变化”。其实这涉及符号混淆，简单来说，在给出运动方程前当然\textit{不知道}正路 $q(\sigma)$。$\dot L\ne 0$ 但仍可以 $\pdv*{L}{\sigma}=0$。

泛函求导法称为\textbf{变分法}。设正路 $q^i(\sigma)$ 存在，任取旁路 $\hat q^i(\sigma)$，但差值 $\delta q^i(\sigma):=\hat q^i(\sigma)-q^i(\sigma)$ 严格遵循 $\delta q^i(a)=\delta q^i(b)=0$。
这称为 \textbf{Fréchet 变分}。因 $\delta\sigma=0$，则即使 $\pdv*{L}{\sigma}\ne 0$ 也仍有 $\pdv{L}{\sigma}\delta\sigma= 0$。
泛函变分指其增量的线性主部。注意 Fréchet 变分无关于 $\sigma$ 及其端点，因此可与参数的微分、积分交换，且代数性质与微分相似。最小作用量原理指出正路上恒有 $\delta S=0$，即 $S$ 增量总属于 $o(\delta q^i)$。
...
对称性是当代物理的重要概念。圆具有左右、旋转对称等，实质是圆在反射、旋转变换下长相没变。考虑单粒子 $m$ 的体系。假设粒子在时间 $\Delta t$ 后运动了 $\bm v\Delta t$。为判断粒子之后如何运动，可将整个系统沿反方向 $-\bm v\Delta t$ 平移，注意体系（的作用量）具有空间平移对称性，因此新体系的 $m$、初始条件等同于旧体系，从而运动相同的 $\bm v\Delta t$，相当于旧体系将一直保持 $m\bm v$ 守恒。同理，设想一根杆子自由转动，由于体系具有空间旋转对称性，因此它将保持角动量守恒。
可见，对称性可以导致守恒量，并约束相互作用。

考虑对 $S=\int L(t,q^i,\dot q^i)\d{t}$ 进行变换 $q^i\xrightarrow{\varphi}\tilde q^i,t\xrightarrow{\varphi}\tilde t$，可简写为 $q^i(t)\to\tilde q^i(\tilde t)$。
...
$q^i(t)\to\tilde q^i(\tilde t)$ 下路径的改变为\textbf{等时变分} $\bar\delta q^i(t)=\tilde q^i(t)-q^i(t)$，参数的改变 $\delta t=\tilde t-t$。这里的变分称为\textbf{Noether 变分}，注意与前文区分\footnote{当然，等时变分和 Fréchet 变分的代数性质是一致的。}。之后将 $q^i(t)$ 简写为 $q^i$。无穷小时即 $\delta t =\epsilon \eta(t, q^i, \dot{q}^i), \bar\delta q^i =\epsilon \xi^i(t,q^i,\dot{q}^i)$。
...
\eq{
    \dv{Q}{t}=0,\quad  Q:=\frac{1}{\epsilon}\left(\pdv{L}{\dot q^i}\delta q^i+L \delta t\right)=\pdv{L}{\dot q^i} \xi+L \eta.
}
对于多参数变换的推广是类似的。
可见\textit{在壳时连续整体对称性导致相应的守恒量}，这称为\textbf{Noether 定理}。但注意并非所有守恒量都来源于某个对称性。

对阐释常见的能量、动量来讲，Noether 定理是足够的。
若时间不做变换 $\eta=0$，则守恒量为 $Q=p_i \xi^i$，其中 $p_i:=\pdv*{L}{\dot{q}^i}$ 称为\textbf{正则动量}。

现在取 $q^i$ 为惯性系坐标 $x^i$，由 $L=\frac 12\sum\dot x^i\dot x_i$


当 $\xi^i$ 为均匀的单位矢量场 $\hat{e}^i$ 时，就是沿 $\hat{e}^i$ 方向的空间平移，于是 $Q=\hat{\bm e}\cdot \sum_{i=1}^{N} \boldsymbol{p}_{i}$ 就是系统的线动量在 $\hat{e}^i$ 方向上的分量。

正则动量即线动量 $\boldsymbol{p}_{i}=\frac{\partial L}{\partial \dot{\boldsymbol{x}}_{i}}=m\bm u$。 

在空间单位法矢 $\bm n$ 的平面上的无穷小整体转动为 $\delta \bm x_i=\theta\bm n\times\bm x_i,\delta t=0$，故\textbf{空间各向同性}的守恒量为
\eq{
    Q=\frac{1}{\theta} \sum_{i=1}^{N}\bm p_i \cdot \delta \bm x_i=\bm n\cdot\sum_{i=1}^{N} \bm L_i,\quad \bm L_i:=\bm x_i\times\boldsymbol{p}_{i},
}
即总角动量在 $n$ 方向上的分量守恒。


严格来说，对称变换指的是位形空间上的 $\delta q$，而不是重参化 $\delta t$。$\delta t$ 之所以也被称作对称变换，是因为其通过 $\Delta q=\delta q+\dot q\delta t$ 导致的 $\delta q$ 是对称变换。若系统具有\textbf{时间平移对称性}，其中时间的无穷小整体平移记作 $\delta t=\epsilon$，且 $q=\bm x$ 在该重参化下满足 $\Delta q=0$，因此 $\delta q=-\dot q\delta t=-\epsilon\dot q$，
时间平移对称性的守恒量为
\eq{
    H:=-\frac{1}{\epsilon}\left(\sum_{i=1}^{N}\bm p_i \cdot \delta\bm x_i+L\delta t\right)=\sum_{i=1}^{N}\bm p_i \cdot \dot{\bm x}_i-L.
}
...
位形空间上的\textbf{点变换}。
这说明点变换和坐标变换十分相似。区别在于，前者是点变、坐标系不变（\textbf{主动观点}），后者是点不变、坐标系变（\textbf{被动观点}）。
