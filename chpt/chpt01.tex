...
先用光线研究时空的因果结构。
令 $\d s^2=0$，则 $t=\pm\left(r+2M\ln|r-2M|\right)+C$。
考虑 $r>2M$，上式正负号取正后斜率为正，为外向族，负号对应内向族。
注意到内向族分别在 $r>2M,0<r<2M$ 单调，可设想直接将其映射为向内 $45^\circ$ 斜率的直线 $\tilde t=-r+C$，且为了 $r=0$ 时 $t=\tilde t$，可取
\eq{
    \tilde t=t+2M\ln\abs{\frac{r}{2M}-1}\implies \d s^2=-\left(1-\frac{2 M}{r}\right) {\d \tilde t}^2+\frac{4M}{r}\,\mathrm{d} \tilde t\, \mathrm{d} r+\left(1+\frac{2 M}{r}\right)\d r^2,
}
现在度规分量在视界处收敛，且 $g=-(g_{01})^2\ne 0$，这才算消去奇性。
...
其二正是 $\dv{\tilde t}{r}=-1$ 或 $\tilde t=-r+C$。
显然前者为外向族而后者为内向族。
