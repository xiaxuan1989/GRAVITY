...
这称为该度规的 \textbf{Killing 方程}。满足该方程的矢量场 $\xi^\mu$ 称为 \textbf{Killing 矢量场}。点变换观点总可转述为坐标变换观点，故实际上对等度规变换的寻找可转化为对 Killing 矢量场的寻找。
...
\begin{align}
    \square_g f&:=\nabla_\mu\nabla^\mu f=\frac{1}{\sqrt{-g}} \p_\mu\left(\sqrt{-g} g^{\mu\nu}\p_\nu f\right),\\ 
T^{\mu \nu}{}_{;\nu}&=T^{\mu \nu}{}_{,\nu}+\Gamma_{\lambda \nu}^\mu T^{\lambda \nu}+\Gamma_{\lambda \nu}^\nu T^{\mu \lambda}
...
于 $(0,r_0)$ 释放自由落体质点 $Z^\mu$。测地线方程为
\[L=\frac{1}{2}g_{\mu\nu} Z^\mu Z^\nu\implies\dv{\tau}(g_{\mu\nu} Z^\nu)=\frac 12 g_{\alpha\beta,\mu} Z^\alpha Z^\beta.\]
在 Schwarzschild 系下 $g_{\mu\nu,0}=0$，则 $g_{0\nu} Z^\nu=\text{常数}$。
或者说因为 $\xi_\mu Z^\mu$ 守恒，取 $t$ 方向 Killing 矢量 $\xi^\mu = \pdv*{x^\mu}{t}$，而在 Schwarzschild 系下 $\xi_\mu Z^\mu=g_{0\nu} Z^\nu$。
