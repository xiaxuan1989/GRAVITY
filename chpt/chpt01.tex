...
\end{enumerate}
$U^\mu$、$U_\mu$、Kronecker 符号都是 Lorentz 张量。偏导保持 Lorentz 协变，故 4-加速 $U^\nu\del_\nu U^\mu$ 也是 Lorentz 张量。
...
称为\textbf{4-动量}。由时空对称性，其分量分别解释为动量 $p^i=P^i$（称为\textbf{3-动量}）和能量 $E=P^0=p_i u^i-L$。
将 $\gamma$ 按二项式定理展开 $p^i=Eu^i=\gamma m u^i=\left(m+\frac{1}{2} mu^{2}+\cdots\right)u^i$。
可见若 $u\ll 1$，则 3-动量近似于经典动量 $m u^i$，即 3-动量给出了其相对论修正。$E=\gamma m$ 即我们悉知的\textbf{质能方程}，经典动能 $(1/2)mu^{2}$ 的出现表明 $(\gamma-1)m$ 是其相对论修正。可见质点静止时也有能量，其应能释放并得到应用，对核能的开发已验证了这一点。
...
定义$\varphi:=A^0,a^i:=A^i$，分别解释为电标势和磁矢势。$A_\mu$ 称为\textbf{4-势}，规范自由性导致 $\varphi+C,\bm a+\grad f$ 的选取自由。可见，对称性催生电荷 $J^\mu$，规范化催生电磁场 $A_\mu$。Noether 荷 $\int J^0\d[3]x$ 表示 $\phi$ 所携带的带电\textit{数}，再配合耦合常数 $e$ 就得到了电量。
