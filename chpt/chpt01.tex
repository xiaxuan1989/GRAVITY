...
\begin{figure}[ht]
    ...
    \includegraphics[width=.28\textwidth]{fig/chpt01/reflecting.pdf}
    }%
    \qquad
    \subfloat[\centering 动系对钟]{\includegraphics[width=.4\textwidth]{fig/chpt01/reflec2.pdf}
    }%
    \caption{\small 若以 $G$ 为绘图基准，只需使反射光与 $G$ 夹角呈 $45^\circ$ 即可使 $BD,AC$ 正交，结果如(a)。以该系为基准，研究一个运动惯性观者的对钟情况，结果如(b)。从图上看，动系坐标轴不是欧氏正交，但被约定为时空上的正交。当然，并非所有参考系都能办到满足时空正交的对钟。}
    \label{fig:clock_syn}
\end{figure}
...
\begin{figure}[ht]
    ...
    \caption{\small 作某事件的同时面可投影得其坐标时。设静系 $S$ 和动系 $S'$。如图，某事件的 $t$ 坐标可做 $S$系同时面截得，而 $t'$ 坐标可做$S'$系同时面截得。两个事件在一个惯性系同时，但在另一惯性系中不同时。无法区别出特定的同时面，此即\textbf{同时的相对性}的直观图像。}
    \label{fig:simurela}
\end{figure}
...
固有时间 $\Delta\tau$ 即观者线的线长，故 $\tau$ 又称\textbf{线长参数}。线性函数 $a\tau+b=\lambda$ 统称\textbf{仿射参数}(affine parameter)。
光对其世界线上的事件标记为一致坐标时，即对光而言不能定义固有时。光不能充当观者。描述光线可用坐标时 $t$ 参数化。

切矢按模方符号可类似分为三类。光线的切矢一定类光。类光矢量亦称\textbf{零模}(null)矢量。所有类光矢量之集 $C_N$ 称为\textbf{光锥}(light cone)。
\begin{figure}[ht]
    \centering
    \subfloat[\centering \small 3 维时空的光锥]{
    ...
    \quad
    \subfloat[\centering\small 因果结构]{\includegraphics[width=.3\textwidth]{fig/chpt01/structure cone.pdf}
    }
    \caption{光锥}
\end{figure}
由三维 Cauchy-Schwartz 不等式，若 $V^\mu$ 类时，而 $W^\nu$ 是非零类时或类光矢量，则当且仅当在任意惯性系（变换满足 ${\Lambda^{0}}_{0} >0$）中满足 $V^{0} W^{0}>0$ 时有 $V^\mu W_\mu<0$。因此可在光锥及其内部定义等价关系，称 $V^\mu,W^\nu$ 具有相同的\textbf{时间指向}。容易发现存在两种等价方式。规定 $V^0>0$ 是\textbf{指向未来的}(future directed)矢量，那另一种方式指定的矢量就称为\textbf{指向过去的}(past directed)。忽略掉零矢量，光锥被分成了两部分，即\textbf{未来光锥}和\textbf{过去光锥}。一般而言，只在讨论某些反演对称性时才考虑指向过去的惯性系。类空矢量没有未来或过去的分类。
...
“boost”取时间轴上的推动之意，但可见一个较好称呼是\textbf{伪转动}(pseudo rotation)：
$t'= t\cosh\theta - x\sinh\theta, x'= x\cosh\theta -t \sinh\theta$。
如图 \ref{fig:boost}，光速相对于日常情形较大，因此图上光线应“下压”，则空间轴转动不明显，退回 Galileo 变换，大家共用绝对时间。快度概念比速度更简洁。比如，设 $u_1=\tanh\theta_{1}$ 是 $B$ 相对于 $A$ 的速度，而 $u_2=\tanh\theta_{2}$ 是 $C$ 相对于 $B$ 的速度，可证 $C$ 相对于 $A$ 的速度是 $u={(u_{1}+u_{2})}/{(1+u_{1} u_{2})}=\tanh(\theta_1+\theta_{2})$，此即速度叠加公式。
...
\begin{figure}[p]
    ...
    \caption{四类变换的图像直观}\label{fig:boost}
\end{figure}
...
粒子受外场作用，而场自身的演化也要服从最小作用量原理。以粒子情形类比，场的位形取相应空间场，但场的路径应是空间场往时间的延伸，即正是时空上的场 $\psi(x)$，如张量场，而位形是某 $\Sigma_t$ 上的截取 $\psi(t,\bm x)$。
...
\eq{
    Q=\frac{1}{\epsilon} \sum_{i=1}^{N}\frac{\partial L}{\partial \dot{\boldsymbol{x}}_{i}} \cdot \delta_s \bm x_i=\hat{\bm e}\cdot \sum_{i=1}^{N} \boldsymbol{p}_{i},\quad \boldsymbol{p}_{i}:=\frac{\partial L}{\partial \dot{\boldsymbol{x}}_{i}},
}
...
其中考虑到 3-加速在 3-速上投影为 $\dot{u}$；模方为 $A^\mu A_\mu=\gamma^4 a^ia_i+\gamma^6(u^ia_i)^2$。故 $A^\mu=0$ 时任意坐标 3-加速都为零。
...
\eq{
p_i=\pdv*{L}{u^i}=\gamma m u_i\To p^i=\gamma m u^i=m u^i+\frac{1}{2} m u^{2}u^i+\cdots,
}
...
\eq{
    P_\mu=\pdv*{L}{\dot x^\mu}=m U_\mu\To P^\mu=m U^\mu.
}
...
若间隔类光或类时，则各惯性系将一致认同两个事件的先后顺序，因为 Lorentz 变换具有正时性。但对于类空间隔，其事件的坐标时差距可以在不同惯性系下取任意实数。即存在一些观者判定两个事件同时，另一些观者则断言 $p$ 先于 $q$，剩下的观者反之。这在图 \ref{fig:simurela}, \ref{fig:spacelike} 中一目了然。类空间隔的事件先后次序没有意义。类时或类光线统称\textbf{因果线}(causal curve)，能用因果线连接的事件正是具有因果联系的事件。在 $\R^4$ 中这等价于用非类空间隔连接。数学上不妨设曲线分段光滑，并使用高数里的中值定理即可。
...
\begin{figure}[ht]
        \centering
        \includegraphics[width=.5\textwidth]{fig/chpt01/HYPERBOLOIDS.pdf}
        \caption{校准线}
\end{figure}
    ...
    \begin{figure}[ht]
        ...
        \includegraphics[width=.45\textwidth]{fig/chpt01/TIME DILATION S.pdf}
        }%
        \qquad
        \subfloat[\centering 在 $S'$ 中静止]{\includegraphics[width=.45\textwidth]{fig/chpt01/TIME DILATION S'.pdf}
        }
        \caption{钟慢效应}
    \end{figure}

    用时空图的校准线，钟慢效应便一目了然。例如，两个观者都看到 $S$ 系原点上的时
    \begin{figure}[ht]
        \centering
        \includegraphics[width=.35\textwidth]{fig/chpt01/INVARIANT HYPERBOLOIDS.pdf}
        \caption{\small $S'$ 系的钟根据同时面 $t'=t_1,t'=0$ 认为 $S$ 的钟慢}
    \end{figure} 
    钟读数“0”，但直线 $t'=t_1$ 与该时钟世界线（$t$ 轴）在 $\left(t,x\right)=(t_1,0)$ 下方的 $(t_1/\gamma,0)$ 相交。
    
    沿 $S$ 系 $x$ 轴静置尺子的情况由时空图道尽。

    \begin{figure}[ht]
        \centering
        \subfloat[\centering 在 $S'$ 中静止]{
        \includegraphics[width=.38\textwidth]{fig/chpt01/INVARIANT 2.pdf}
        }%
        \qquad
        \subfloat[\centering 在 $S$ 中静止]{\includegraphics[width=.4\textwidth]{fig/chpt01/INVARIANT 1.pdf}
        }
        \caption{\small 尺缩效应}
    \end{figure}
...
\end{wrapfigure}
    ...
    尺缩效应导致了有趣的\textbf{车库佯谬}\footnote{英语为 ladder paradox，由 Rindler 提出，最初是借梯子和谷仓(barn)举例的；还可用火车和隧道。}。
...
\eq{
\mathcal L= -\frac 12\del^\mu\phi\del_\mu\phi-U(\phi)\To \square\phi-\pdv*{U}{\phi}=0,
}
...
\eq{
    E_i=F_{i0},\quad B_i=\varepsilon_{ijk}\del^j A^k=\varepsilon_{ijk}\del^{[j}A^{k]}=\frac 12 \varepsilon_{ijk}F^{jk}\Leftrightarrow F^{ij}=\varepsilon^{ijk}B_k.
}
可使用矩阵表示为
...
为消除规范自由度，可约定\textbf{规范固定}(gauge fixing)条件，简称规范条件。比如\textbf{Lorenz 规范}\footnote{指丹麦物理学家 Ludvig Valentin Lorenz(1829-1891)。当然它又是 Lorentz 协变的，所以也可叫做 Lorentz 规范。二人名字的发音相同，容易混淆，需要注意 Lorentz 是荷兰的物理学家，相对论的奠基者之一。}
\eq{
    \del_\mu A^\mu =0.
}
...
我们说 $S'$ 系相对于 $S$ 的速度是 $u_i=\dv*{x_i}{t}$，是指 $S'$ 任意的相对静止的世界线在 $S$ 下测得的速度分量都是 $u_i$。显然这等价于研究 $S'$ 系的空间原点运动。反之，设 $S$ 系的空间原点在 $S'$ 下测得 $u'_i=\dv*{x'_i}{t'}$。先考虑 $S'$ 的空间原点，其世界线始终满足 $\d x'_i=0$，则 $\d x_\lambda=(\Lambda^{-1})_{\lambda 0}\d x'_0$，故 $\Lambda_{00}=(\Lambda^{-1})_{00}=\dv*{t}{t'},(\Lambda^{-1})_{i0}=u_i\dv*{t}{t'}$，
    ...
    \eq{\label{eq:Au'=u}
        \delta_{ij}\Lambda_{i0}\Lambda_{jk}=\Lambda_{00}\Lambda_{0k}\To \Lambda_{jk} u'_j=-\gamma u_k.
    }
...
\eq{
    \delta\int\d\tau=\delta\int\sqrt{-\eta_{\alpha\beta}\d\xi^\alpha\d\xi^\beta}=0\To \tilde A^\alpha=\dv{\tilde{U}^\alpha}{\tau}=\dv[2]{\xi^\alpha}{\tau}=0.
}
...
则
\eq{
    \sgn g=-1\To 1/\tilde J=\sqrt{-g}\To S=\int \mathcal L \sqrt{-g}\d[4]{x},
}
这里不妨假设坐标变换保持右手性。可见 $g$ 虽与坐标选择有关，但在变换下一定保号，即引力度规的行列式总是负的（无关号差）。规定\textbf{适配体元} $D^4 x$ 只在局域惯性右手系 $\{\xi\}$ 中表为 $\d[4]\xi$，就有 $D^4 x=\sqrt{-g}\d[4]{x}$。
...
\eq{
\delta\int \left(-m+q A_\mu U^\mu\right)\d\tau =0 \To \frac{D P^\mu}{\d\tau}=q F^\mu{}_{\nu}U^\nu,
}
电磁场的运动方程遵循
\eq{
    \delta\int\left(-\frac{1}{16\pi} F^{\mu\nu} F_{\mu\nu}+A_\nu J^\nu\right)\sqrt{-g}\d[4]{x}=0 \To \nabla_\mu F^{\mu\nu}=-4 \pi J^\nu,
}
而 $F_{\mu\nu}$ 的定义和无挠性自动给出 $F_{\mu\nu}=2\nabla_{[\mu}A_{\nu]}=2\del_{[\mu}A_{\nu]}$，则
\eq{
    F_{[\mu\nu;\lambda]}=F_{[\mu\nu,\lambda]}=0.
...
\eq{
    \d s^2=-\left(\frac{1-M/2\rho}{1+M/2\rho}\right)^2\d t^2+\left(1+\frac{M}{2\rho}\right)^4(\d \rho^2+ \rho^2\d\Omega^2).
}
