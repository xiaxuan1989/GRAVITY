...
其中 $L$ 称作\textbf{拉氏量}(Lagrangian)，$T$ 即总动能。通俗地说，这种函数的函数称为\textbf{泛函}(functional)\footnote{设函数 $f:A\to\R$。$A=\R^n$ 时说明自变量可由 $n$ 个独立实数描述，称\textbf{自由度}为 $n$。曲线的点无限多，因此泛函可视作函数在\textbf{无穷自由度}下的极限。泛函的自变量称为\textbf{宗量}。}，通常表为积分形式，如曲线的线长。按 $L=T-V$ 设定的 $S(\bm x_a(t))$ 就给出关于 $\bm x_a(t)$ 的经典力学方程。
...
当然，根据 E-L 方程可直接看出
\[
\dot p_i=\pdv{L}{q^i},\quad \dot H=\pdv{L}{\dot q^i}\ddot q^i+\dot q^i\dv{t}\pdv{L}{\dot q^i}-\dot L=-\pdv{L}{t}.
\]
...
称为\textbf{4-速}。显然 4-速是单位类时切矢：$\eta_{\mu\nu}U^\mu U^\nu=-1$。规定 $U_\mu:=\eta_{\mu\nu}U^\nu$，就还可紧凑写作 $U^\mu U_\mu = -1$。这称为矢量在度规下的\textbf{对偶矢量}(dual vector)。
定义 $(\eta^{\mu\nu}):=(\eta_{\mu\nu})^{-1}$。$\eta_{\mu\nu}$ 作为广义单位阵，易证数值上 $(\eta_{\mu\nu})=(\eta^{\mu\nu})$。故还可对偶回去：$U^\mu=\eta^{\mu\nu}U_\nu$。
$\dot{x}^i$ 称为\textbf{3-速}，其对偶为 $\dot{x}_i=\eta_{ij}\dot{x}^j=\delta_{ij}\dot{x}^j$，模长即 $u:=\sqrt{\dot{x}^i \dot{x}_i}$。可见 4-速分量为 $U^0=\gamma,U^i=\gamma \dot{x}^i$。
同理，\textbf{4-加速}为
\eq{
    A^\mu:=\dv[2]{x^\mu}{\tau}=U^\nu\p_\nu U^\mu.
}
...
可见 4-加速分量为 $A^0=\gamma \dot{\gamma}=\gamma^4 u\dot{u}=\gamma^4 \dot{x}^i\ddot{x}_i, A^i=\gamma^2 \ddot{x}^i+\gamma^4 \dot{x}^j\ddot{x}_j \dot{x}^i$，
其中考虑到 3-加速在 3-速上投影为 $\dot{u}$；模方为 $A^\mu A_\mu=\gamma^4 \ddot{x}^i\ddot{x}_i+\gamma^6(\dot{x}^i \ddot{x}_i)^2$。故 $A^\mu=0$ 时任意坐标 3-加速都为零。固有系中 $\dot{x}^i=0$，故 4-加速等于固有系 3-加速。
...
这里\textit{坐标系可以是任意的}。
当然，作为物理规律的 $\Lag$ 一般不显含 $x^\mu$。在粒子情形我们通过决定性原理排除高阶导，由于现在 $\p_\mu$ 还包含相互作用势的含义，不能冒然排除高阶：事实上 $\Lag$ 的确可能存在高阶导 $\Phi_{,\cdots}$，则 E-L 方程一般需要修改；但若高阶导只是表面上看起来存在，实际可通过某种方式消除（马上将看到），则 E-L 方程照旧。实操中总是直接计算变分，即可规避“E-L 方程可能错误”这种麻烦。
...
\]
并注意 $\bar\delta(\Phi_{,\nu})=\p_\nu(\bar\delta \Phi),\bar\delta x^\mu=0$。我们知道体元变换 $\d[4]\tilde x=J\d[4]x$，其中 Jacobi 行列式为
\[
...
\eq{
    \mathcal H=S^{00}=\pi\dot\Phi-\Lag,\quad \pi = \pdv{\Lag}{\dot\Phi},
}
其中 $\pi$ 为\textbf{共轭动量密度}，$\dot\Phi=\p_t\Phi$。$S^{0i}=\pi\delta^i\Phi$ 就是\textbf{动量密度}。
...
Newton 时空的对称性是分开的，即时间平移、空间平移（含转动）。
但在相对论中，时空对称性是时空一体的。
...
注意 $U^\mu=\gamma\dot x^\mu$，则 $L=m U^\mu U_\mu/\gamma=m U_\mu \dot x^\mu$，所以惯性系下的正则动量为
\eq{
    P_\mu=\pdv{L}{\dot x^\mu}=m U_\mu\im P^\mu=m U^\mu,
}
称为\textbf{4-动量}。由时空对称性，其分量分别解释为动量 $p^i=P^i$（称为\textbf{3-动量}）和能量 $E=P^0=p_i \dot{x}^i-L$。
将 $\gamma$ 按二项式定理展开 $p^i=E \dot{x}^i=\gamma m \dot{x}^i=\left(m+\frac{1}{2} mu^{2}+\cdots\right)\dot{x}^i$。
...
另一方面 $-E^2+p^2=P^\mu P_\mu$ 或者 $E^2= p^2+m^2$，此即相对论性\textbf{色散关系}(dispersion relation)。
...
$F^\mu:=\dv*{P^\mu}{\tau}$ 称\textbf{4-力}，分量 $F^0=\gamma\dot{E},F^i=\gamma \dot{p}^i$，其中 $\dot{p}^i$ 即\textbf{3-力}。质量守恒时 $F^\mu=m A^\mu,\dot{p}^i=m\ddot{x}^i$，功能关系 $\dot{p}^i \dot{x}_i=\dot{E}$ 得到保留。
...
正如运动方程是 $x^\mu(\tau)$ 的常微分方程，场方程是 $g_{\mu\nu}$ 的偏微分方程，且根据广义不变性，它应是张量方程，其中的张量由度规及其偏导构造。自由引力场、物质最小引力耦合的总和记 $\Lag=\frac{1}{2\kappa}\Lag_G +\Lag_M$，其中 $\kappa$ 是按习惯设置的待定耦合常数。尽管可在局域惯性系中消除引力，但作为物理规律的作用量是不能在局域惯性系中失去耦合性的，它必须是标量场。取局域惯性系 $\{\xi\}$ 并置 $S=\int \Lag \d[4]{\xi}$。
...
最小耦合指出可取 $\Lag_G=R$。下面计算变分。为简便运算，选择对 $g^{\mu\nu}$ 而非 $g_{\mu\nu}$变分，则 $\var(R\sqrt{-g})$ 即涉及 $g^{\mu\nu},R_{\mu\nu},\sqrt{-g}$ 三个部分。
...
\begin{align}
    \square_g f&=\frac{1}{\sqrt{-g}} \p_\mu\left(\sqrt{-g} \cd^\mu f\right)=\frac{1}{\sqrt{-g}} \p_\mu\left(\sqrt{-g} g^{\mu\nu}\p_\nu f\right),\\ 
T^{\mu \nu}{}_{;\nu}&=T^{\mu \nu}{}_{,\nu}+\Gamma_{\lambda \nu}^\mu T^{\lambda \nu}+\Gamma_{\lambda \nu}^\nu T^{\mu \lambda}
=\frac{1}{\sqrt{-g}} \p_\nu\left(\sqrt{-g} T^{\mu \nu}\right)+\Gamma_{\lambda \nu}^\mu T^{\lambda \nu},
\end{align}
进而 $\square_g x^\lambda=(-g)^{-1/2}\p_\mu(\sqrt{-g} g^{\mu\lambda})=-g^{\mu\nu}\Gamma^\lambda_{\mu\nu}$。
...
将该结果用于分部积分：
\begin{align*}
    2\int \phi \cd_\lambda(g^{\mu [\nu} \delta \Gamma_{\mu \nu}^{\lambda]}) D^4x &\simeq -2\int  g^{\mu [\nu} \delta \Gamma_{\mu \nu}^{\lambda]} \cd_\lambda \phi D^4x\\
    &= -\int  (g_{\mu\nu}\cd^\lambda (\delta g^{\mu\nu})\cd_\lambda \phi - \cd_\mu(\delta g^{\mu\lambda}) \cd_\lambda \phi)  D^4x\\
    &\simeq \int  (g_{\mu\nu} \delta g^{\mu\nu}\cd^\lambda \cd_\lambda \phi - \delta g^{\mu\lambda} \cd_\mu\cd_\lambda \phi)  D^4x\\
...
&=  \int\bigg(f_R R_{\mu \nu} -\frac{1}{2} f(R) g_{\mu \nu} \bigg) \delta g^{\mu \nu}D^4x+2\int f_R \cd_\lambda(g^{\mu [\nu} \delta \Gamma_{\mu \nu}^{\lambda]}) D^4x\\
