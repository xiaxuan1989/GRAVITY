...
这称为\textbf{红移量}，$\sqrt{-\xi_\mu\xi^\mu}$ 称为红移因子。若 $\{x\}$ 的坐标时就是稳态时间，$\xi^\mu=\delta^\mu_0$，则 $\xi_\mu\xi^\mu=g_{00}$。若 $r_r>r_e$，则 $z>0$，光将红移，否则蓝移。但这并非因为光爬出引力势阱而消耗能量。被高处静止观测者测得的频率更低。

取 Schwarzschild 度规，则
\eq{
    z=\sqrt{\frac{1-2GM/r_r c^2}{1-2GM/r_e c^2}}-1.
}
近似下得到 $z=-\Delta\phi$
