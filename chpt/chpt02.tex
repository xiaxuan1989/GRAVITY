...
设置两个稳态观者，一者为光源 $e$，一者为吸收端 $r$。

Killing 矢量 $\xi^\mu = \pdv{x^\mu}{x'^\nu}\delta^\nu_0 = \pdv*{x^\mu}{t}$， $t$ 为 Schwarzschild 时间。知 $k^\mu\xi_\mu$ 沿光路为常量。

Schwarzschild 系下，角频率为 $\omega=-k_\mu U^\mu=-k_\mu \frac{\delta^\mu_0}{\sqrt{-g_{00}}}=-\frac{k_0}{\sqrt{-g_{00}}}=-\frac{k_\mu\xi^\mu}{\sqrt{-g_{00}}}$，波长为 $\lambda = $。

所以\textbf{红移量}为
\eq{
    z=\frac{\lambda_r}{\lambda_e}-1=\sqrt{\frac{g_{00}(x_r)}{g_{00}(x_e)}}=\frac{\omega_r}{\omega_e}
}
