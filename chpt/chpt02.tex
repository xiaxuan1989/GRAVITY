...
设置两个稳态观者，一者为光源 $e$，一者为接收端 $r$。二者 4-速共线于 Killing 矢量 $\xi^\mu = \pdv{x^\mu}{x'^\nu}\delta^\nu_0 = \pdv*{x^\mu}{t}$，$t$ 为稳态时间。知 $k^\mu\xi_\mu$ 沿光路为常量。
角频率为 $\omega=-k_\mu U^\mu=-k_\mu\xi^\mu/\sqrt{-\xi_\mu \xi^\mu}$，而波长为 $\lambda = 2\pi/\omega$。故
\eq{
    z=\frac{\lambda_r}{\lambda_e}-1=\sqrt{\frac{\xi_\mu\xi^\mu |_r}{\xi_\mu\xi^\mu |_e}}-1,
}
这称为\textbf{红移量}，$\chi = \sqrt{-\xi_\mu\xi^\mu}$ 称为红移因子。若 $\{x\}$ 的坐标时就是稳态时间，$\xi^\mu=\delta^\mu_0$，则 $\xi_\mu\xi^\mu=g_{00}$。
取 Schwarzschild 度规，则
\[z=\sqrt{\frac{1-2M/r_r}{1-2M/r_e}}-1.\]
若 $r_r>r_e$，则 $z>0$，光将红移，否则蓝移。但这并非因为光爬出引力势阱而消耗能量。被高处静止观测者测得的频率更低。
弱场近似得 $z=-\Delta\phi,\phi=-M/r$，考察地表处，则 $z=g\Delta h$。


最著名的实验是 Pound-Rebka 实验，该实验在 1959 年由 Robert Pound 和 Glen A. Rebka 在哈佛大学进行。他们使用了一个高塔和一个伽马射线源，测量了从塔顶到塔底的伽马射线频率变化，验证了引力红移的预测。
...
可以预料，若对 Schwarzschild 系坐标时 $t$ 做推迟变换，即得超前 Eddington 系的时间反演坐标，故称\textbf{推迟 Eddington 系}，它描述的就是黑洞的反演：物质从视界穿出，物理现象朝着与黑洞性质相反的方向进行。从图上看，IV 区沿类时方向可以走向 A 区。物理学家将 IV 区打趣地称为\textbf{白洞}(white hole)，简称 \textbf{W 区}。同理\textbf{白洞视界}记 $\H_V^-,\H_U^-$。可见，推迟 Eddington 系覆盖了 $\mathrm{A}\cup\H_U^-\cup\mathrm{W}$。最后还有一个 III 区，从度规上看，其也是一个渐近平直视界外时空，而它亦可从 W 区穿出而坠入 B 区，鉴于相似性又称 III 区为 \textbf{A$'$ 区}，然而此二区域不可能有任何信息交流，即\textbf{无因果联系}，因为从 A 出发的任意类时或类光线都不能进入 A$'$，反之亦然。于是物理学家戏称 A$'$ 为另一独立的“宇宙”，二者互为\textbf{平行宇宙}(parallel universes)。如果硬要说二者有“擦边性”的瓜葛，那只能在原点 $V,U=0$ 处。还原另二空间维度，它就是一个半径 $r=2M$ 的 2 维球面，称为\textbf{虫洞}(wormhole)、\textbf{喉}(throat)或\textbf{分叉点}(bifurcation)，以代表两个“宇宙”唯一可能但却不能通过的道路。这些概念是在全时空为真空的前提下所得，其物理实在性还须另做讨论。从初值问题的角度考虑，整个时空存在的可能性很小。
