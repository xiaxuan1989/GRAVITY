...
\[X'^\mu=\sum_{\nu=0}^3 (A^{\mu}{}_{\nu}+B^{\mu}{}_{\nu})X^\nu=\sum_{\nu=0}^3 A^{\mu}{}_{\nu}X^\nu+\sum_{\nu=0}^3 B^{\mu}{}_{\nu}X^\nu.\]
$A^{\mu}{}_{\nu}X^\nu,B^{\mu}{}_{\nu}X^\nu$ 都出现 $\nu$。替换哑元后也可不重复，写作 $A^{\mu}{}_{\nu}X^\nu,B^{\mu}{}_{\sigma}X^\sigma$。单个因式可以出现多对哑标，比如，用 $\delta_{ij}$ 表示单位阵元（称为\textbf{Kronecker 符号}），则 Descartes 系中线元的勾股定理是
...
\]
$({\partial x'^\mu}/{\partial x^\nu})$ 就是分析学里的 \textbf{Jacobi 矩阵}。注意对于 Jacobi 矩阵元，一般不用简记符 $\p_\nu x'^\mu$。
