...
\begin{tikzpicture}[scale=2.5]
  ...
  \def\Nlines{14} % number of world lines (at constant x/t)
  \def\DNxp{-8}   % difference in number of world lines of x' axis (shorten)
  \def\DNyp{-4}   % difference in number of world lines of y' axis (shorten)
  ...
  \pgfmathsetmacro\wb{4*\d} % width of barn in S
  \pgfmathsetmacro\yb{(\xb+\wb-\L)/tan(\ang)} % y = ct coordinate when ladder is full in barn in S
  \coordinate (L) at (\L,0); % ladder end in S
  \coordinate (L') at (\ang:\Lz); % ladder end in S'
  \coordinate (A) at (90-\ang:{\yb/cos(\ang)}); % left end of ladder when fully in barn
  \coordinate (B) at ($(A)+(\L,0)$); % right end of ladder when fully in barn
  \coordinate (C) at (90-\ang:{\yb/cos(\ang)+(\wb)/sin(\ang)}); % left end of ladder when fully passed through barn

  
  % FILL
  \begin{pgfonlayer}{back} % draw on back
    \fill[mydarkblue!22] % barn
      (\xb,\yminb) rectangle (\xb+\wb,\ymax);
    \draw[->,thick,mydarkblue] (\xb,\yminb) -- (\xb,\ymax+0.15);
    \draw[->,thick,mydarkblue] (\xb+\wb,\yminb) -- (\xb+\wb,\ymax+0.15);
    \fill[mylightred] % ladder
      (90-\ang:-\xmin) -- (90-\ang:\ymaxp) --++ (\ang:\Lz) -- (L) --++ (90-\ang:-\xmin) -- cycle;
    
    
  \end{pgfonlayer}
  \draw[->,thick,mydarkbrown] (L)++(90-\ang:-\xmin) -- (L) -- (L') --++ (90-\ang:\ymaxp+0.2);
  
  % LADDER

  
  % LADDER IN BARN
  \draw[rod] (A)++(L) --++ (\ang:-\Lz) node[pos=0.5,sloped,above,scale=0.8] {\contour{mylightred}{\small 车静长}};
  \draw[rod] (A) --++ (L) node[pos=0.5,above,scale=0.8] {\small 车动长};
  ...
  \node[mydarkblue,below=0,align=center,scale=0.8,yshift=1] at (\xb+\wb/2,0)
    ...
    \draw[thick,samples=100,smooth,variable=\y,domain=-1.2:.5]
    plot({\xb+\wb-sqrt((\wb)^2+(\y)^2)},\y+\yb);
